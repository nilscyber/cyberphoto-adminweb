...
*/
	...
	// om vi inte har någon annan moms än 25% (eller 22%) gör vi med vanliga 
	...
	if ($row->rabatt_moms3 == 0 AND $row->rabatt_moms0 == 0) {
		...
		if ($row->rabatt_moms1 > 0 ) {
			if ($row->fland_id == 358 || $row->fland_id == 999)  {
				if ($row->lang == "fi") {
					array_push($goodslist, mk_goods(1, "rabatt", "Alennus (Alv 22%)", -$row->rabatt_moms1*100, $momssats, 0));
				} else {			
					array_push($goodslist, mk_goods(1, "rabatt", "Rabatt (22% moms)", -$row->rabatt_moms1*100, $momssats, 0));
				}			
			} else {
 ...
 */
	...
	if ($orderrow->fland_id == 358) {
		if ($orderrow->lang == "sv")
			array_push($goodslist, mk_goods_flags(1, "frakt", "Fraktkostnad", $freightCost*100, 22, 0, $KRED_IS_SHIPMENT));
		else
			array_push($goodslist, mk_goods_flags(1, "frakt", "Rahti", $freightCost*100, 22, 0, $KRED_IS_SHIPMENT));
