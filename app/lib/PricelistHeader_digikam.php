<div align="left"><left>
<table onmouseout="ClearHiLite();" border="0" cellpadding="2" width="99%" style="border-collapse: collapse; border: 1px solid #CCCCCC" cellspacing="1" bgcolor="#F7F7F4">
<tr>
<td valign="bottom"><font face="Verdana, Arial" size="1">&nbsp;&nbsp;</font></td>
<% if ($testresult == "yes") { %>
<td align="right" valign="bottom"><font face="Verdana, Arial" size="1">&nbsp;</font></td>
<% } %>
<% if ($sortera == "ccd" || $sortera == "zoom") { %>
<td align="right" valign="bottom"><font face="Verdana, Arial" size="1"><% echo $head; %></font></td>
<% } %>
<% If ($_SESSION['RememberMoms'] == 1) { %>
<td align="right" valign="bottom"><font face="Verdana, Arial" size="1"><b><% if (!$sv): %>Hinta&nbsp;alv&nbsp;0%<% else: %>Pris&nbsp;utan&nbsp;moms<% endif; %></b></font></td>
<% } else { %>
<td align="right" valign="bottom"><font face="Verdana, Arial" size="1"><b><% if (!$sv): %>Hinta&nbsp;alv&nbsp;24%<% else: %>Pris&nbsp;med&nbsp;moms<% endif; %></b></font></td>
<% } %>
<td align="right" valign="bottom" width="45">&nbsp;&nbsp;</td>
</tr>
