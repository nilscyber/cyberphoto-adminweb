...
function viewItemsInBasket($firstbasket, $fi) {
	...
	$bestallningsgrans, $pack, $alltidBrev, $extraFrakt;
	
	$freight_check = NULL;
	$extraFrakt = false; // visar om någon produkt innehåller frakttillägg. Används bl.a. till att tyngre produkter inte skall skickas som hempkaet
	$brev = true;
	$alltidBrev = false;
...
<%		printf("%10.0f SEK", $utpris*$count);  %>
		...
		<td bgcolor="#ECECE6" align=right><font face="Verdana, Arial" size="1">
<%		printf("%10.0f SEK", ($utpris + $utpris * $momssats) * $count); %>
		</font></td>
...
<%		printf("%10.0f", $outprice);  %>
		...
		<td bgcolor="#ECECE6" align=right><font face="Verdana, Arial" size="1">
<%		printf("%10.0f", $outprice + $outprice * $momsts); %>
		</font></td>
