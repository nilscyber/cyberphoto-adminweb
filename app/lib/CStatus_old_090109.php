...
eof;
	...
	function levStatus ($skickat, $behandlat, $levklar, $betalsatt_id, $leveranssatt_id, $faktura_ok, $ordernr, $skickad_av) {
		...
		elseif ($behandlat != "") {
			...
			if ($fi)
				if ($betalsatt_id == 5 && $skickad_av == 'XX' ) { 
					if ($fi && !$sv) {
				...
				//} elseif ($leveranssatt_id == 8 AND $betalsatt_id == 7) { 
					...
					if ($leveranssatt_id == 8) {
						if ($levklar == 3) 
							...
							$output .= "inväntar klartecken från Kreditor Europe AB";						
					} else {
						if ($levklar == 3) 
							$output .= "klar att skicka, skickas inom kort";
						elseif ($levklar == 12) 
							$output .= "ordern är makulerad hos Kreditor Europe AB";
						else
							$output .= "inväntar klartecken från Kreditor Europe AB";						
					}
				}
			...
			if ($betalsatt_id == 4) {
				...
				if ($leveranssatt_id == 8) {
					if ($levklar == 3) 
						...
						$output .= "ej fullständig samt inväntar klartecken från Kreditor Europe AB";						
				} else {
					if ($levklar == 3) 
						$output .= "ej fullständig";
					else 
