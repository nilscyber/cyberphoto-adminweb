...
function process_orderrows() {
	...
	$itemsselect .= "FROM Artiklar, Orderposter, Tillverkare WHERE Orderposter.artnr = Artiklar.artnr AND ";
	...
	while ($row = mssql_fetch_object($itemres)) :
		...
		<td bgcolor="#ECECE6" align=right><font face="Verdana, Arial" size="1">
		<?php printf("%10.0f SEK", $outprice*$count*1.25); ?>
		 </font></td>
		<?php
		print "</tr>\n";
		...
		$messagehtml .= "</td>\n";
		$messageHtml .= "<td bgcolor=\"#ECECE6\" align=\"right\"><font face=\"Verdana, Arial\" size=\"1\">";
		$messageHtml .= sprintf("%10.0f SEK", $outprice*$count*1.25);
		$messagehtml .= "</td>\n";
		...
		$messageText .= sprintf("%10.0f SEK", $outprice*$count*1.25);
		$messageText .= "\n";
		/**
		if ($bokad == 0) {
	...
	$itemsselect  = "SELECT Orderposter.artnr, Artiklar.beskrivning, Orderposter.utpris, Orderposter.antal ";
	$itemsselect .= "FROM Artiklar LEFT JOIN Orderposter ON Orderposter.artnr = Artiklar.artnr ";
	$itemsselect .= "WHERE Orderposter.ordernr = '$ordernrladdaom' AND (Orderposter.artnr like 'frakt%') ";
	...
	while ($row = mssql_fetch_object($itemres)) :
		...
		<td bgcolor="#ECECE6" align=right><font face="Verdana, Arial" size="1">
		<?php  printf("%10.0f SEK", $outprice*1.25); ?>
		</font></td>
		</tr>
		<?php
		  $price = $outprice * 1.25;
		...
		$messagehtml .= "</td>";
		$messageHtml .= "<td bgcolor=\"#ECECE6\" align=\"right\"><font face=\"Verdana, Arial\" size=\"1\">";
		$messageHtml .= sprintf("%10.0f SEK", $outprice*1.25); 
		$messagehtml .= "</td>";
...
if (eregi("pac$", $artnr)) {
	...
	$select = "SELECT artnr, beskrivning, lagersaldo, antal, till_kund, Artiklar.utpris ";
	...
	while ($row = mssql_fetch_object($res)):
		...
		$insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, levDatum, levDatumDatumInlagt) ";
		$insert .= "values ($newordernr, '$artnr2', $newCount, $del_utpris, '$bokad', '$name2', '$levDatum', '$inkommet') ";
	...
	$insert_rabatt = "UPDATE Ordertabell SET rabatt = '$tot_rabatt', kommentar = '$newkommentar' WHERE ordernr = '$newordernr'";
	...
	// Stoppa annars in som vanligt
	  ...
	  $insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, levDatum, levDatumDatumInlagt) ";
	  $insert .= "values ($newordernr, '$artnr', $count, $outprice, $bokad, '$name', '$levDatum', '$inkommet') ";
	  // ändring tillfällig
	  //echo $insert;
	  if ($artnr != "")
	  	mssql_query ($insert);
