...
class Lionite_Paypal {
     ...
     * @var boolean
     */
    protected static $_sandbox = true;
    
    public $_confirmCheckoutPaymentResult;
    public $_confirmCheckoutPaymentCallString;
    public $_confirmCheckoutPaymentCallString2;
	...
	protected $_settings = array (
        ...
        'sandbox' => array(
            //'username' => 'paypal-test_api1.cyberphoto.se',
            //'password' => '1378195813',
            //'signature' => 'A-UvcC8P0dUsCO0NZmHIcMOWWzq5AeBFSL7gCMuJoM57VGVTCM0zh0EI',
            'username' => 'paypal-test3_api1.cyberphoto.no',
            'password' => 'TH4MRPZBAJBTXLBC',
            'signature' => 'An5ns1Kso7MWUdW4ErQKJJJ4qi4-AO80-Nx6L2TDpJ0Sbbm0tDrOXcSJ',
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $n => $item) {
                ...
                if (isset($item['subscription_description'])) {
                    $nvp[] = 'L_BILLINGAGREEMENTDESCRIPTION0=' . urlencode($item['subscription_description']);
    ...
    public function confirmCheckoutPayment($options = array(), $items = array()) {
        ...
        foreach (array('cost', 'currency', 'shipping', 'tax', 'insurance', 'handling', 'item_cost', 'payer_id') as $key) {
        ...
        if (empty($items) && !isset($options['cost'])) {
            $this->_confirmCheckoutPaymentCallString = 'No items or cost provided - cannot determine transaction cost';
        ...
        $this->_confirmCheckoutPaymentCallString2 = $options;
        $nvp = $this->_buildExpressCheckoutNVP($options, $items);

        $nvpstr = 'TOKEN=' . $token . '&PAYERID=' . $payerID . '&IPADDRESS=' . urlencode($_SERVER['SERVER_NAME'])
                . '&' . implode('&', $nvp);
        $this->_confirmCheckoutPaymentCallString = $nvpstr;
        $result = $this->apiCall("DoExpressCheckoutPayment", $nvpstr);
        $this->_confirmCheckoutPaymentResult = $result;
        
        if (  is_array($result) && 
                "success" == strtolower($result['ACK']) && 
                strtolower($result['PAYMENTINFO_0_PAYMENTSTATUS']) == "completed" ) {
            
            return $result['PAYMENTINFO_0_TRANSACTIONID'];
        } else {
            echo "1: " . strtolower($result['ACK']);
            echo "2: " . $result['PAYMENTINFO_0_PAYMENTSTATUS'];
            echo "3: " . $result['PAYMENTINFO_0_TRANSACTIONID'];            
            return false;
        }
    }
