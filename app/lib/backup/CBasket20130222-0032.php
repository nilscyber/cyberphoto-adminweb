...
//if ($_SERVER['REMOTE_ADDR'] == "192.168.1.98") { // inför starten av adempiere
...
function checkSoldLastMonth($artnr) {
	...
	$select = "SELECT antalManad1 FROM mostSoldArticles WHERE artnr = '" . $artnr . "' ";
	$res = mysqli_query($this->conn_my, $select);
	$row = mysqli_fetch_object($res);

	if (mysqli_num_rows($res) > 0) {
		echo $row->antalManad1 . " st/mån";
	}
	else {
		echo "Inga sålda";
	}
...
function add_customer($uppdatera, $mailpassw) {
	...
	if (($pay == "invoiceme" || $pay == "sveainvoice" || preg_match("/sveapayment/i", $pay) || eregi("avbetalning", $pay) || $one_stop == "yes" || $this->mobilabonnemang || $pay == "faktura_kampanj" || $pay == "klarnakonto" || $this->aldersgrans || $_SESSION['MOB_CONFIRMED']) && isset($_SESSION['CKreditor']->cust_addresses) ) {
	...
	*/
	...
	if (!$uppdatera) {
		if ($this->testUsername ($userName)) {
			...
			if ($res && $res2) {
				...
				// 121128, tar bort nedan i två veckor och kollar reaktionerna
	...
	// if ($new_nyhetsbrev == -1)
	if ($new_nyhetsbrev == -1 || ($new_email != "" && $one_stop == "yes")) // 110819 lagt till så att alla personer som handlar via snabbkassan får nyhetsbrevet
	...
	else
	{
	  if ($betalsatt == 'faktura') { $betalsatt_id = 4; }
	    else
	    {
	      ...
	      elseif ($betalsatt == 'sveainvoice') $betalsatt_id = 20;
	      elseif (preg_match("/sveapayment/i", $pay)) $betalsatt_id = 21;
	      else $betalsatt_id = 0; 
	      //foretagshyra
	    }
	}
...
function session_clear() {
	 ...
	 $_SESSION['old_giftcardrebate'],$_SESSION['old_operator'],$_SESSION['old_one_stop'],$_SESSION['old_abbpersonnumber']);
...
function getDiscountText($rowDiscount, $momssats, $utpris, $showDecoration) {
	...
	if ($fi) {
		if ($rowDiscount->discountPercent_fi != "") {		
			...
			if ($sv) {
				$txt = round( ($rowDiscount->discountAmount_fi + $rowDiscount->discountAmount_fi * $momssats)*$count, 0) .  " EUR avdraget enligt rabattkod";
			} else {
				$txt = round( ($rowDiscount->discountAmount_fi + $rowDiscount->discountAmount_fi * $momssats)*$count, 0) .  " EUR alennus";
			}
...
function getDiscountPrice($rowDiscount, $artnr, $kategori_id, $tillverkar_id, $utpris) {
	...
	if ($rowDiscount->discountCode == "SAMSUNG25") {
		if ($tillverkar_id == $rowDiscount->tillverkar_id && ($kategori_id == 448 || $kategori_id == 344 || $kategori_id == 425 || $kategori_id == 427 || $kategori_id == 426 || $kategori_id == 424 || $kategori_id == 338 || $kategori_id == 238 || $kategori_id == 1000144 || $kategori_id == 1000170 || $kategori_id == 704 || $kategori_id == 423)) {
...
function viewItemsInBasket($firstbasket, $fi) {
	...
	$PHP_SELF, $brev, $bestallningsgrans, $pack, $alltidBrev, $pallDelivery, $fi, $sv, $bestallt_fi, $lev_datum_norm_fi, $lev_datum_norm, $release, $discountCode, $discountCodeStatus, $freight, $pay, $articles, $discountAmount, $mobile, $sub;	
	...
	if ($_SERVER['REMOTE_ADDR'] == "192.168.1.89x" && $firstbasket != 'nooutput') {
		//echo $select;
		//echo "här: " . $artnr;
		echo "Utanför: " . $kundvagn . "<br>";
	}
	
	// denna kampanj gäller endast framtill 30 september då Klarna står för alla frakter
	// if (($pay == "klarnakonto" || $pay == "kreditor") && $freight == "fraktpost" && strtotime('2009-09-30 23:59:59') > strtotime("now") && !$fi) {
	if ($pay == "faktura_kampanj" && $freight == "fraktpost" && strtotime('2010-04-30 23:59:59') > strtotime("now") && !$fi) {
		$this->freeFreight = true;
	...
	if ($_SESSION['MOB_CONFIRMED'] && $freight == "fraktpost" && $sjabo == "ejnu") {
		$this->freeFreight = true;
	}

	if ($this->mobilabonnemang && $freight == "fraktpost") {
		$this->freeFreight = true;
	}
	...
	for ($i=0; ($i < $n);  $i+=2) {
		...
		//if ($fi) {
            ...
            $select  = "SELECT Artiklar.artnr, Artiklar.lagersaldo, Artiklar.beskrivning, Artiklar.bestallt, beskrivning_fi, Artiklar.kommentar, Artiklar_fi.kommentar_fi, utpris_fi, utpris, tillverkare, ";
            $select .= "frakt, lagersaldo_fi, bestallt_fi, Artiklar.releaseDate, Artiklar.kategori_id, Artiklar.tillverkar_id,  ";
            $select .= "lev_datum_fi, bestallningsgrans, lev_datum_norm, lev_datum_norm_fi, Artiklar.ej_brev as ej_brev, Kategori.ej_brev as kat_ej_brev, ";
            $select .= "Moms.momssats_fi, Moms.momssats, Artiklar.link, link2_fi, Kategori.kategori_id_parent ";
		...
		*/
		...
		if ($firstbasket == 'nooutput') {
			...
			if ($kategori_id == 509) {
				...
				$_SESSION['MOB_CONFIRMED'] = true;
			} elseif ($tillverkar_id == 3 && $utpris > 5600 && strtotime('2012-12-18 10:50:59') > strtotime("now")) {
				$this->accept_canon = true;
			} else {
		...
		//$extraFrakt = true;
			if ($extra_freight < $frakt) {
				$extra_freight = $frakt; 
				$this->check_extra_freight = true; // detta för att vi skall kunna blockera vissa fraktsätt i kundvagnen
			}
		}
		if ($extra_freight == 3)
			$pallDelivery = true;	
		...
		if ($_SESSION['MOB_CONFIRMED'] && ($_SESSION['MOB_PHONE'] == $artnr)) {
		    ...
		    if ($_SESSION['MOB_FMA'] == 40) {
		    	$description .= " och förhöjd månadsavgift 50 kr/mån";
		    } elseif ($_SESSION['MOB_FMA'] == 80) {
		    	$description .= " och förhöjd månadsavgift 100 kr/mån";
		    } elseif ($_SESSION['MOB_FMA'] == 120) {
		    	$description .= " och förhöjd månadsavgift 150 kr/mån";
		    } elseif ($_SESSION['MOB_FMA'] == 160) {
		    	$description .= " och förhöjd månadsavgift 200 kr/mån";
		    } elseif ($_SESSION['MOB_FMA'] == 200) {
		    	$description .= " och förhöjd månadsavgift 250 kr/mån";
		    } elseif ($_SESSION['MOB_FMA'] == 240) {
		    	$description .= " och förhöjd månadsavgift 300 kr/mån";
		    } else {
		    	$description .= " ";
		    }
		...
		// här visar vi att produkten är en köplats
		if ($_SESSION['KOPLATS_CONFIRMED'] && ($_SESSION['KOPLATS_PRODUCT'] == $artnr)) {

			$description .= "<i><b> Bokas som köplats!</b></i>";
...
?>
			...
			<tr>
			  ...
			  <td bgcolor="#ECECE6"><font size="1" face="Verdana">
			  <?php 
				...
				if ($link <> "")
					...
					{
						if ($fi && !$sv)
							print "<a target=\"_parent\" href=\"http://www.cyberphoto.fi/?info_fi.php?article=".$artnr."\">";
						elseif ($fi && $sv)
							print "<a target=\"_parent\" href=\"http://www.cyberphoto.fi/?info_fi_se.php?article=".$artnr."\">";
						else 
							print "<a target=\"_parent\" href=\"http://www.cyberphoto.se/?info.php?article=".$artnr."\">";
					}
			...
			<td align="center" bgcolor="#ECECE6"><font size="1" face="Verdana">	
				...
				<a onMouseOver="this.T_BGCOLOR='#FFFFFF';this.T_WIDTH=315;return escape('Klickar du här kommer abonnemanget raderas. Även telefonen kommer tas.')" style="text-decoration: none" href="/confirm_abonnemang.php?ID=<?php echo session_id(); ?>&remove=true">
				ta bort</a>

				<?php } elseif ($artnr == "digitalfotokr" && $_SESSION['PRENUM_CONFIRMED']) { ?>
				
				<a onMouseOver="this.T_BGCOLOR='#FFFFFF';this.T_WIDTH=315;return escape('Tar bort erbjudandet om prenumeration för 1kr')" style="text-decoration: none" href="/confirm_prenumeration.php?ID=<?php echo session_id(); ?>&remove=true">
				ta bort</a>
				...
				<a onMouseOver="this.T_BGCOLOR='#FFFFFF';this.T_WIDTH=215;return escape('Tar bort produkten från kundvagnen')" style="text-decoration: none" HREF="javascript:changeItem('<% echo $artnr; %>', 0, 0); kundvagnReload();">
			...
			<td align="left" bgcolor="#ECECE6"><font size="1" color="#008000" face="Verdana">
				<?php		if (!(eregi("^frakt", $artnr)) && $kategori_id != 629) { 
			...
			<td align="right" bgcolor="#ECECE6"><font size="1" face="Verdana">
			...
			<td align="center" width="15"><font size="1" face="Verdana">	
				...
				<a onMouseOver="this.T_BGCOLOR='#FFFF00';this.T_WIDTH=215;return escape('Tar bort produkten från kundvagnen')" style="text-decoration: none" HREF="javascript:changeItem('<% echo $artnr; %>', 0, 0); kundvagnReload();">
				<img border="0" src="/kundvagn/trash.gif"></a>
				<?php } else { ?>							
				<a onMouseOver="this.T_BGCOLOR='#FFFFFF';this.T_WIDTH=350;return escape('Denna kan du inte ta bort, välj annat leveranssätt istället')" style="text-decoration: none">
	...
	if ($_SESSION['CYBAIRGUN_XBOX'] && $products_cybairgun < 4000) { // detta för att säkra upp alla skurkar som försöker fippla
	    unset ($_SESSION['CYBAIRGUN_XBOX']);
	}
	
	if ($this->checkStoreStatus("xbox360cybair") && $firstbasket != 'nooutput' && $products_cybairgun > 4000 && strtotime('2013-01-02 11:00:00') > strtotime("now")) {
	
	    $select  = "SELECT Artiklar.beskrivning, Artiklar.kommentar, utpris, Artiklar_fi.beskrivning_fi, Artiklar_fi.utpris_fi, Moms.momssats, Moms.momssats_fi, Tillverkare.tillverkare ";
		$select .= "FROM Artiklar ";
		$select .= "INNER JOIN Tillverkare ON Artiklar.tillverkar_id = Tillverkare.tillverkar_id ";
		$select .= "INNER JOIN Kategori ON Artiklar.kategori_id = Kategori.kategori_id ";
		$select .= "INNER JOIN Moms ON Artiklar.momskod = Moms.moms_id ";
		$select .= "LEFT JOIN Artiklar_fi ON Artiklar.artnr = Artiklar_fi.artnr_fi ";
		$select .= "WHERE artnr='xbox360cybair'";
		...
		if ($fi && !$sv) {
	    	...
	    	$name = $row->tillverkare . " " .$row->beskrivning . " <b><font color=#85000d>På köpet!</font></b>";
	...
	?>
		<tr>
		  ...
		  <td bgcolor="#ECECE6"><font size="1" face="Verdana"><?php echo "<a target=\"_parent\" href=\"http://www.cyberphoto.se/info.php?article=xbox360cybair\">$name</a>"; ?></td>
		  <td align="center" bgcolor="#ECECE6"><font size="1" face="Verdana">1</td>
		  ...
		  <?php } else { ?>
		  <td align="right" bgcolor="#ECECE6"><font size="1" face="Verdana"><?php echo number_format(($outprice + $outprice * $momsts) * $count, 0, ',', ' ') . " " . $val; ?></td>
		  <?php } ?>
		  <?php if ($_SERVER['REMOTE_ADDR'] == "192.168.1.89") { ?>
		  <!--
		  <td align="center" width="15">
		  <a onMouseOver="this.T_BGCOLOR='#FFFFFF';this.T_WIDTH=350;return escape('Denna kan du inte ta bort, välj annat betalssätt istället')" style="text-decoration: none">
		  <img border="0" src="/kundvagn/trash_no.gif"></a>
		  </td>
		  -->
		  <?php } ?>
	...
	// här projecerar vi det som har med mobilabonnemang att göra
	...
	if ($fi) {
	    ...
	    if ($_SESSION['MOB_FMA'] == 40) {
	    	$abonnemangbeskrivning .= " med förhöjd månadsavgift 50 kr/mån";
	    } elseif ($_SESSION['MOB_FMA'] == 80) {
	    	$abonnemangbeskrivning .= " med förhöjd månadsavgift 100 kr/mån";
	    } elseif ($_SESSION['MOB_FMA'] == 120) {
	    	$abonnemangbeskrivning .= " med förhöjd månadsavgift 150 kr/mån";
	    } elseif ($_SESSION['MOB_FMA'] == 160) {
	    	$abonnemangbeskrivning .= " med förhöjd månadsavgift 200 kr/mån";
	    } else {
	...
	?>
		...
		<a onMouseOver="this.T_BGCOLOR='#FFFF00';this.T_WIDTH=215;return escape('Tar bort produkten från kundvagnen')" style="text-decoration: none" href="/confirm_abonnemang.php?ID=<?php echo session_id(); ?>&remove=true">
		<!-- <img border="0" src="/kundvagn/trash.gif"></a> -->
		fimpa</a>
	...
	*/
	...
	// nedan tar vi fram aviavgift som skall läggas till för Klarna faktura
	
	if ($firstbasket != 'nooutput' && ($pay == "invoiceme" || $pay == "sveainvoice") && ((!$fi && $this->basketValue < 3200) || ($fi && $this->basketValue < 327))) {
	...
	?>
		<tr>
		  ...
		  <td bgcolor="#ECECE6"><font size="1" face="Verdana"><?php echo $name; ?></td>
		  <td align="center" bgcolor="#ECECE6"><font size="1" face="Verdana">1</td>
		  <td bgcolor="#ECECE6"><font size="1" face="Verdana">&nbsp;</td>
		  <td bgcolor="#ECECE6"><font size="1" face="Verdana">&nbsp;</td>
		  <?php if ($_SESSION['RememberMoms'] == 1) { ?>
	...
	if ($extra_freight && $firstbasket == 'yes' && $firstbasket != 'nooutput' && $extra_freight != 999 && $extra_freight != 3 && $freight != Null && !$this->freeFreight) {
	    ...
	    $select  = "SELECT Artiklar.beskrivning, Artiklar.kommentar, utpris, Artiklar_fi.beskrivning_fi, Artiklar_fi.utpris_fi, Moms.momssats, Moms.momssats_fi  ";
...
?>	    
	...
	<tr>
		...
		<?php if ($fi && !$sv) { ?>
		<a onMouseOver="return escape('<b>Suuri lähetys lisämaksu</b><br>Kooltaan suurien tai painavien tuotteiden lähetyskuluihin tuleva lisämaksu (esim. tulostimet, scannerit, jalustat, taustakankaat, suuret laukut jne) Postin taulukon mukaisesti, tällä hetkellä 6,9 Eur (sis. alv 23%).')">
		<?php } else { ?>
...
function check_lager_se($artnr) {
	...
	$est_delivery, $fi, $sv, $release, $web_adempiere;
				...
				if ($release != "") {
					...
					if (strtotime($release) >= mktime()) {
						...
						if (!$sv) // TODO: finska nästa rad
							print "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\"><a onMouseOver=\"return escape('Tuote on tilaustavara. <br>Normaali toimitusaika on <b>$lev_datum_norm </b> päivää varastoomme<br>Tämä toimitusaika vaatii että toimittajalla on tuote varastossa')\" style=\"text-decoration: none\"> Tilaustuote</></font></a>";													
						else
...
function check_lager_se_new($artnr) {
	global $count, $lagersaldo, $lagersaldo_fi, $bestallt, $bestallt_fi, $lev_datum, $lev_datum_fi, $lev_datum_norm, $lev_datum_norm_fi, $package_stock, $bestallningsgrans, $bestallningsgrans_fi, $queue, 
	$est_delivery, $fi, $sv, $release, $web_adempiere, $mobilsite;
	...
	if ($package_stock_sv) {
	...
	} elseif (ereg("pac$",$artnr) && !$package_stock_sv) {
		if (!$sv) {
			print "<a href=\"javascript:levStatusPaket('$artnr', $count, '$fi', '$sv') \" onMouseOver=\"return escape('Tuote on paketti joka koostuu eri osista. Yksi tai useampi osa on loppunut varastosta. Klikkaa nähdääksesi joka tuotteen varastotilaa. Luonnollisesti voit tilata paketin kuitenkin.')\" style=\"text-decoration: none\"><font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">lisää tietoja tästä</a>";
		} else {
			if ($mobilsite) {
				print "<span class=\"notinstore\">Tillfälligt slut</span>";
			} else {
				// print "<a href=\"javascript:levStatusPaket('$artnr', $count, '$fi', '$sv') \" onMouseOver=\"return escape('Varan består av ett paket med flera delar. En eller flera av delarna är slut i lager. Klicka för att se lagerstatus på varje enskild vara. Naturligtvis går paketet bra att beställa ändå. ')\" style=\"text-decoration: none\"><font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\" >klicka för info</a>";
				print "<a href=\"javascript:levStatusPaket('$artnr', $count, '$fi', '$sv') \" style=\"text-decoration: none\"><font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\" >klicka för info</a>";
				// print "<span onclick=\"show_hide('$artnr');\" style=\"cursor:pointer;\"><font face=\"Verdana, Arial\" size=\"1\" color=\"#85000D\">klicka för info</font></span>";
			}
		}
		...
		$web_adempiere->check_lager($artnr,1000000,$count); // här kollar vi alla andra möjligheter
	}         
}
...
function getPriceWithVat($artnr) {
	...
	$select = "SELECT utpris, Moms.momssats, Moms.momssats_fi FROM Artiklar ";
        $select .= "INNER JOIN Moms ON Artiklar.momskod = Moms.moms_id ";
			...
			if ($antal_sum >= $totalNeed) {
				...
				if ($ber_lev_datum == "" || $ber_lev_datum == "1900-01-01" || $ber_lev_datum == "2100-01-01") {
					...
					elseif (!$sv)
						...
						$articleinfo = "<a onMouseOver=\"return escape('Leveransdatum okänt innebär att vår leverantör i dagsläget inte har något beräknat leveransdatum för när de får in varan till sig. Så fort vi får ett nytt leveransbesked uppdateras detta.')\" style=\"text-decoration: underline\"> leveransdatum okänt</a>";
				} else {					
					//$prec = 1;
					//echo "här: " . $table_fi;
					/**
...
function showDeliveryDate($dat, $prec, $fi, $sv) {
	...
	*/
	...
	} else { // detta ska aldrig behöva inträffa, men vi kör den ändå
		return $dat;
	}
...
//		echo $bestallt_fi;
		...
		<tr>
		  ...
		  <?php if($ej_med != -1 && $kategori_id != 486) { ?>
		  	...
		  	<?php } ?>
		  <?php } else { ?>
		  	<a onMouseOver="return escape('Det finns ingen produktsida för denna artikel!')" style="text-decoration: none">
		  <?php } ?>
		  <?php if ($bild != "") { ?>
...
function formatDiscountTxt($rs, $showstyle) {
	...
	*/
	} elseif ($showstyle == 3) { // Detta visar endast texten kampanj, till för prislistorna där utrymmet är begrännsat
...
function viewPacketDeliveryMorePackages($artnr, $mangd, $fi) {
	...
	if (mysqli_num_rows($res) > 0) {
		...
		<tr>
		  ...
		  <td valign="top"><font color="#2B2B2B" face="Verdana, Arial" size="1"><a onMouseOver="this.T_WIDTH=350;return escape('<b><% echo $fulldescription; %></b><br><br><img border=\'0\' src=\'/thumbs/xlarge/bilder/<% echo $bild; %>\'>')" href="<% echo $linkto; %>"><?php echo $description; ?></a></font></td>
