...
function show_digikam($tekniska_data, $extra_articles) {
	...
	$select  = "SELECT Artiklar.artnr, beskrivning, tillverkare, ej_med, link FROM Artiklar, Tillverkare, Tekniska_data ";
	//$select .= "LEFT JOIN Tillverkare ON Artiklar.tillverkar_id=Tillverkare.tillverkar_id ";
	$select .= "WHERE Artiklar.tillverkar_id = Tillverkare.tillverkar_id AND Artiklar.artnr = Tekniska_data.artnr ";

	$select .= " AND Artiklar.kategori_id = 6 && ej_med=0 AND (utgangen=0 OR lagersaldo > 0) && AND demo != -1 && ((Artiklar.artnr not like '%pac') || (Artiklar.artnr like '%pac' && artnr like 'QV2800UXpac'))";
