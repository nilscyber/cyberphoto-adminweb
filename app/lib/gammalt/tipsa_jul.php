...
<html>
<head>
<title>Skicka ett julklappstips!</title>
</head>
<body>
<% echo $extra; %>
<b><font face="Arial" color="#85000D">Skicka ett julklappstips!</font></b><p><b>
...
<table border="0" cellpadding="2" style="border-collapse: collapse" bordercolor="#111111" id="AutoNumber1">
  ...
  <tr>
    ...
    <input type="submit" value="skicka tips" style="font-size: 8pt; font-weight: bold; font-family: Verdana"></font></td>
  </tr>
  </table>
...
<% } else { 
	...
	<script language="javascript">
		document.location.replace("sent_jul.php?res=true");
	</script>

	<% } else { %>
	
	<script language="javascript">
		document.location.replace("sent_jul.php?res=false");
	</script>
	<% } 
...
function renderHtmlMail($row, $beskrivning, $betygLink, $tipSrc, $messSrc) {
	ob_start();        
	include ("./base/tipsa_email_jul.htm"); 
	...
	if ($row->link != "") {
		...
		$link = "http://www.cyberphoto.se/?info_jul.php?article=" . $row->artnr;	
	}
	if ($row->bild != "") {
...
function sendMail($row, $from, $to, $messageHtml, $messageText, $conn_ms, $artnr) {
	...
	$mail->Subject = "Jultips från " . $from;	
