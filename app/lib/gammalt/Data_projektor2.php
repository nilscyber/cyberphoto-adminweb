<%
include_once ("CTekniska_data_proj.php");
...
if ($comparepage != 'yes') { 
	...
	$select = "SELECT * FROM Data_projektor WHERE artnr = '$tekniska_artnr[1]'";
	...
	for ($i = 1; $i <= $num_of_articles; $i++): 
		...
		$select[$i] = "SELECT * FROM Data_projektor WHERE artnr = '$tekniska_artnr[$i]'";
...
<tr>
	<td valign="top" bgcolor="#ECECE6"><font face="Verdana, Arial" size="1">Strömförsöjrning</font></td>
	<% for ($i = 1; $i <= $num_of_articles; $i++): if ($i > '5') break;  %>
