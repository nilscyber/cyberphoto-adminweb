...
function write_formline($artnr, $momssats, $momskod) {
	...
	global $newordernr, $count, $outprice, $name, $lagersaldo, $complete_order, $till_kund, $inkommet, $moms1, $moms2, $moms3, $moms4;	
 	// Nollställ variabler
 	$totalpacutpris = "";
  	$rabatt = "";
	$check = "";
	...
	if (eregi("pac$", $artnr)) {
		...
		while ($row = mssql_fetch_object($res)):
			...
			if ($row->momskod == 1)
				$moms1 +=  round((($row->utpris * $row->antal  * $count - $Totalt) * $row->momssats),2); 
			if ($row->momskod == 2)
				$moms2 +=  round((($row->utpris * $row->antal * $count - $Totalt) * $row->momssats),2); 
			if ($row->momskod == 3)
				$moms3 +=  round((($row->utpris * $row->antal * $count - $Totalt) * $row->momssats),2); 
			if ($row->momskod == 4)
				$moms4 +=  round((($row->utpris * $row->antal * $count - $Totalt) * $row->momssats),2); 
				

		    } else {
			$s = "UPDATE Orderposter set paketRabatt = " . $andelKr . " + paketRabatt WHERE ordernr = " . $newordernr . " AND artnr = '" . $row->artnr . "'";
			if ($row->momskod == 1)
				$moms1 +=  round((($row->utpris * $row->antal * $count - $andelKr) * $row->momssats),2);
			if ($row->momskod == 2)
				$moms2 +=  round((($row->utpris * $row->antal * $count - $andelKr) * $row->momssats),2); 
			if ($row->momskod == 3)
				$moms3 +=  round((($row->utpris * $row->antal * $count - $andelKr) * $row->momssats),2);
			if ($row->momskod == 4)
					$moms4 +=  round((($row->utpris * $row->antal * $count - $andelKr) * $row->momssats),2); 
		...
		// Stoppa annars in som vanligt
		  ...
		  $insert .= "values ($newordernr, '$artnr', $count, $outprice, $bokad, '$name', '$levDatum', '$inkommet', $momssats, $momskod) ";
			...
			if ($momskod == 1)
				$moms1 += ($momssats*$outprice*$count);
			elseif ($momskod == 2)
				$moms2 += ($momssats*$outprice*$count);
			if ($momskod == 3)
				$moms3 += ($momssats*$outprice*$count);
			if ($momskod == 4)
				$moms4 += ($momssats*$outprice*$count);
...
function process_orderrows() {
	...
	endwhile;
		...
		<tr>
		  <td bgcolor="#ECECE6" align="left"><font face="Verdana, Arial" size="1">Paketrabatt</font></td>
		  <td bgcolor="#ECECE6" align="right"><font face="Verdana, Arial" size="1">1 st</font></td>
		  <td bgcolor="#ECECE6" align="right"><font face="Verdana, Arial" size="1"><?php printf("%10.0f SEK", $orderrow->paketRabatt); ?></font></td>
		  <td bgcolor="#ECECE6" align="right"><font face="Verdana, Arial" size="1"><?php printf("%10.0f SEK", $orderrow->paketRabatt); ?></font></td>
		</tr>
		<?php
		$messageText .= "  1   ";
