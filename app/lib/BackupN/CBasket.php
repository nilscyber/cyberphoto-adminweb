...
"old_nyhetsbrev", "CcName", "old_avtalskund", "old_mobilnr", "old_sms", "old_nyhetsbrev", "old_forsakringserbjudande", "senasteLevsatt", "senasteBetalsatt", "pay", "freight", "pallDelivery");
...
function viewItemsInBasket($firstbasket, $fi) {
	...
	$PHP_SELF, $brev, $bestallningsgrans, $pack, $alltidBrev, $pallDelivery, $fi;
	...
	for ($i=0; ($i < $n);  $i+=2) {
		...
		$select .= "Moms.momssats, Artiklar.link ";
...
function check_lager($artnr, $fi) {
	global $count, $lagersaldo, $bestallt, $lev_datum, $lev_datum_normal, $package_stock, $bestallningsgrans, $queue, $est_delivery, $fi;
	...
	if (!(ereg("frakt",$artnr))) {
		...
		if ($lagersaldo >= $count || ($package_stock == '1') )  {
			...
			if ($bestallt >= $neededStock) {
				...
				echo "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">". $this->kollaLevtid ($artnr, $count, NULL, $fi);
				...
				if (ereg("pac$",$artnr))
					{
						...
						if ($fi == 'yes')
							...
							print "<a href=\"javascript:levStatusPaket('$artnr', '$count') \"  onMouseOver=\"return escape('Varan består av ett paket med flera delar. En eller flera av delarna är slut i lager. Klicka för att se lagerstatus på varje enskild vara. Naturligtvis går paketet bra att beställa ändå. ')\" style=\"text-decoration: none\"><font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\" >klicka för info</a>";
				...
				elseif ($bestallningsgrans == '0')
					{
						...
						if ($fi == 'yes')
							...
							print "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\"><a onMouseOver=\"return escape('Produkten tas hem på beställning. <br>Normal leveranstid är <b>$lev_datum_normal</b> till vårt lager<br>Denna leveranstid kräver att vår leverantör har varan på lager')\" style=\"text-decoration: none\"> beställningsvara</></font>";
					}
				else
					{
						if ($fi == 'yes')
							...
							print "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\"><a onMouseOver=\"return escape('Produkten är tillfälligt slut i lager. Normal leveranstid <b>$lev_datum_normal</b>')\" style=\"text-decoration: none\"> tillfälligt slut</font></a>" ;
					}
				}
		}
	}
