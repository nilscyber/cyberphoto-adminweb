...
function show_old_order($kundnr) {
	...
	$select = "SELECT ordernr, inkommet, skickat, behandlat FROM Ordertabell WHERE kundnr = '$kundnr' && show_order = '0' order by ordernr DESC";
...
#			" (klicka för att följa paketet)";
		...
		if ($fi == 'yes') {
			return "<a href=\"http://server.logistik.posten.se/servlet/PacTrack?kolliid=" . $row->ParcelNo . "$kollinr&xslURL=/xsl/pactrack/standard.xsl&lang=SE&cssURL=http://www.cyberphoto.se/css/pacsoft.css" . "\">" . $row->ParcelNo . "</a>" . 
				" (Klikkaamalla tästä voit seurata pakettia)";
		}
		else {
			return "<a href=\"http://server.logistik.posten.se/servlet/PacTrack?kolliid=" . $row->ParcelNo . "$kollinr&xslURL=/xsl/pactrack/standard.xsl&lang=SE&cssURL=http://www.cyberphoto.se/css/pacsoft.css" . "\">" . $row->ParcelNo . "</a>" . 
				" (klicka för att följa paketet)";
...
<%		if (!(eregi("^frakt", $artnr))):
			...
			elseif ($bokad == 4) {
				if ($fi == 'yes')
					...
					echo "<font color=\"#85000D\">Produkten är utgången! Vänligen <a href=\"mailto:produkt@cyberphoto.se?subject=Utgången produkt $artnr $beskrivning\"><u><font color=\"#85000D\">kontakta oss</font></u></a> för alterantiv</font>";
			}
