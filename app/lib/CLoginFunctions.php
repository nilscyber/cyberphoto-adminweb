...
function viewPacketDelivery($artnr, $mangd, $fi) {
	...
	if (mysqli_num_rows($res) > '0') {
		...
		<tr>
		  <td bgcolor="#ECECE6"><font color="#2B2B2B" face="Verdana, Arial" size="1"><% echo $description; %></font></td>
		  <td bgcolor="#ECECE6" align="center"><font color="#2B2B2B" face="Verdana, Arial" size="1"><% echo $antal*$mangd; %></font></td>
		...
		<tr>
		  ...
		  <a href="mailto:order@cyberphoto.se"><% if ($fi == 'yes'): %>Information saknas, kontakta cyberphoto för mer info<% else: %>Information saknas, kontakta cyberphoto för mer info<% endif; %></a>
		  </font></td>
		</tr>
		<%
	}
...
<%	}
	...
	if ($extra_freight && $firstbasket == 'yes' && $firstbasket != 'nooutput'): 
	    ...
	    else $extra_freight_arntr = 'frakt+'; // för säkerhets skull
	    $select = "select beskrivning, kommentar, utpris from Artiklar where artnr='$extra_freight_artnr'";
...
%>	    
		...
		<td bgcolor="#ECECE6" align=right><font face="Verdana, Arial" size="1">
<%		printf("%10.0f", $outprice);  %>
		</font></td>
		<td bgcolor="#ECECE6" align=right><font face="Verdana, Arial" size="1">
<%		printf("%10.0f", $outprice*1.25); %>
		</font></td>
...
function check_lager($artnr, $fi = false) {
	global $count, $lagersaldo, $bestallt, $lev_datum, $lev_datum_normal, $package_stock, $bestallningsgrans, $queue, $est_delivery, $sv;
	$lager = "";
	...
	if (!(ereg("frakt",$artnr))) {
		...
		if ($lagersaldo >= $count || ($package_stock == '1') )  { 
			...
			$lager .= "<font size=\"1\" face=\"Verdana, Arial\" color=\"#385F39\">";
			...
			if ($bestallt >= $neededStock) {
				...
				$lager .= "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">".kollaLevtid ($artnr, $count, NULL, $fi);
			...
			<%
				...
				if (ereg("pac$",$artnr))
					{  
						...
						if ($fi == 'yes' && !$sv)
							$lager .= "<a href=\"javascript:levStatusPaket('$artnr', '$count') \"><font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">lisää tietoja tästä</a>";
						else
							$lager .= "<a href=\"javascript:levStatusPaket('$artnr', '$count') \"><font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">klicka för info</a>";
					}
				elseif ($bestallningsgrans == '0') 
					{  
					
						if ($fi == 'yes' && !$sv)
							$lager .= "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">tilaustuote, normaali toimitusaika $lev_datum_normal";
						else
							$lager .= "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">beställningsvara, normal leveranstid $lev_datum_normal (till vårt lager)";
					}
				else
					{ 
						if ($fi == 'yes' && !$sv)
							$lager .= "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">tilapäisesti loppunut, normaali toimitusaika $lev_datum_normal" ;
						else
							$lager .= "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\">tillfälligt slut, normal leveranstid $lev_datum_normal (till vårt lager)" ;
					}
				}
		}         
	}
...
function check_lager2($artnr, $lagersaldo, $bestallt, $lev_datum, $lev_datum_normal, $bestallningsgrans, $fi) {
		global $count, $lagersaldo, $bestallt, $lev_datum, $lev_datum_normal, $bestallningsgrans;
		$count = 1;
		return check_lager($artnr, $fi); 
...
function kollaLevtid ($artnr, $count, $ordernr, $fi)  { // artnr som skall kollas, antal samt inkommet datum på ordern
	...
	
	{
		...
		if ($fi == 'yes')
			...
			$articleinfo = "varan är beställd men levererantören har inte kunnat ge en specifik leveranstid";	
	}
...
if ($articleinfo == "" || $articleinfo == "-") {
	if ($fi == 'yes')
		...
		$articleinfo = "varan är beställd men leveranstid är okänd";
}
return $articleinfo;
}
