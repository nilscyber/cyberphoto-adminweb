...
Class CKreditor {
	...
	function get_payment_period($pay) {
		...
		if ($pay != "kreditor") {
			...
			elseif ($pay == "avbetalning15")
				$this->payment_period = 15;
			elseif ($pay == "avbetalning24")
				$this->payment_period = 24;
			elseif ($pay == "avbetalning36")
				$this->payment_period = 36;
			elseif ($pay == "klarnakonto")
				$this->payment_period = 88;				
			elseif ($pay == "avbetalningjul09")
				$this->payment_period = 89;				
			elseif ($pay == "faktura_kampanj")
				$this->payment_period = 98;				
			elseif ($pay == "avbetalning_canon2012")
				$this->payment_period = 90;				
			elseif ($pay == "avbetalningjul12")
				$this->payment_period = 91;				
			else 	
	...
	function reserve_amount() {
		...
		if ($fi) {
			if ( strlen($_SESSION['old_co']) > 2 ) 
				$addr = $_SESSION['old_co'];
			else 
				$addr = $_SESSION['old_ladress'];							
				
			$fadr = mk_address($_SESSION['old_firstName'], $_SESSION['old_lastName'], $addr, $_SESSION['old_postnr'], $_SESSION['old_postadr'], $cntry);
		} else {
		...
		//exit;
		$this->cust_pno = trim($this->cust_pno);
		$status = reserve_amount($this->cust_pno, $goodsvalueMoms*100, "", "", "", 
						$this->orderId, $fadr, $fadr, $_SESSION['old_email'], $_SESSION['old_telnr'], $_SESSION['old_mobilnr'],
		...
		*/
		...
		if ($status != 0) {
			...
			$insert = "INSERT INTO logWeb (logDate, logPage, logComment, logBasket, logDeny, logReason) values (now(), 'kreditor', 'Personnr:  " . $this->cust_pno . ". Kundnummer: $kundnrsave. Ordersumma: $goodsvalueMoms. pclass: $this->pclass', '$kundvagn_local', '1', '$result')";
			...
			$insert = "INSERT INTO logWeb (logDate, logPage, logComment, logBasket) values (now(), 'kreditor', 'resultat: " . $result . ". Personnr:  " . $this->cust_pno . ". kundnummer: $kundnrsave. ordersumma: $goodsvalueMoms. pclass: $this->pclass', '$kundvagn_local')";
		}
			
                
		// $updt = "INSERT INTO log_web (dat, webpage, comment, artnr) values ('" . $date . "', 'kreditor', 'resultat: " . $result . ". Expl: " . $this->kreditor_result_text . ".  Personnr:  " . $this->cust_pno . ". kundnummer: $kundnrsave. ordersumma: $goodsvalueMoms. pclass: $this->pclass. kundvagn: $kundvagn_local', '" . $artnr . "')";
		// echo $updt . "<br>";
		// @mssql_query($updt, $conn_standard);
		// $insert = "INSERT INTO logWeb (logDate, logPage, logComment, logBasket) values (now(), 'kreditor', 'resultat: " . $result . ". Expl: " . $this->kreditor_result_text . ".  Personnr:  " . $this->cust_pno . ". kundnummer: $kundnrsave. ordersumma: $goodsvalueMoms. pclass: $this->pclass. kundvagn: $kundvagn_local', '$kundvagn_local')";

		// $insert = "INSERT INTO logWeb (logDate, logPage, logComment, logBasket) values (now(), 'kreditor', 'resultat: " . $result . ". Expl: " . $this->kreditor_result_text . ".  Personnr:  " . $this->cust_pno . ". kundnummer: $kundnrsave. ordersumma: $goodsvalueMoms. pclass: $this->pclass', '$kundvagn_local')";
		mysqli_query($conn_my2, $insert);
	...
	function getSetOrderId() {
		...
		$res = mysqli_query($conn_standard, $str);
		if (mysqli_num_rows($res)>0) {
			$row = mysqli_fetch_object($res);
			$orderId = $row->maxId + 1;
			mysqli_query("INSERT INTO orderIds (orderId) values (" . $orderId . ")"); 
	...
	function print_addresses_mobilesite() {
		...
		if (!isset($this->cust_addresses))
			return;		
		//$this->cust_addresses[0][0] . " " . $this->cust_addresses[0][1]
		$ret =  "<input class='inputaddressgrey' type='text' name='namn' size='23' value='" . $this->cust_addresses[0][0] . " " . $this->cust_addresses[0][1] . "' onFocus='this.blur()'><br>";
		$ret .=  "<input class='inputaddressgrey' type='text' name='adress' size='23' value='" . $this->cust_addresses[0][2] . "' onFocus='this.blur()'><br>";
		$ret .=  "<input class='inputaddressgrey' type='text' name='postnummer' size='5' value='" . $this->cust_addresses[0][3] . "' onFocus='this.blur()'>";
	...
	function get_monthly_cost($goodsValueMoms,$flags) {
		global $fi;
	
		$goodsValueMoms = ($goodsValueMoms*100);
		$months = 24;
		// $flags = 0;
		if ($fi) {
			$rate = 22.0;
		...
		$status = monthly_cost($goodsValueMoms, $rate, $months, $monthsfee, $flags, $currency, &$result);
		...
		if ($pay == "invoiceme") {
			...
			if ($fi) {
				...
				switch ($pay) {
					...
					$pclass = 1583;
					break;
					// case(15): // denna gäller vår 15-månaders kampanj
					// $pclass = 1226;
					// break;
					case (3):
					$pclass = 105;
					...
					break;					
					case (90):
					$pclass = 4587; // canon kampanj 2012 (julen)
					break;					
					case (99):
					$pclass = 264;
					break;					
