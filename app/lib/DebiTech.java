...
public class DebiTech {
	...
	public static void main(String[] args) {
		...
		// kontrollera om ordernummer är medskickat eller inte
		// om det inte är meskickat, boka upp ett.
		String string;
		string = args[12];
		long ordernr;
		boolean newOrder;
		if (string.length() == 0) {
			newOrder = true;
			ordernr = getOrdernr();
			if (ordernr == 0) { // om det blir fel, pröva två gånger till för säkerhets skull.
				ordernr = getOrdernr();
			}
			if (ordernr == 0) { // andra gången
				ordernr = getOrdernr();
			}
			...
			newOrder = false;
			ordernr = Long.parseLong(args[12]);
		}

		DebiTech debitech = new DebiTech();
		/*
	...
	public static long getOrdernr () {
			...
			try {
					...
					if (result.wasNull() ) {
						...
						maxNr += 1;
						strInsert = "INSERT INTO Ordertabell (ordernr, inkommet) values (" + maxNr + ", '" + Tid.nu() + "')";
						try {
