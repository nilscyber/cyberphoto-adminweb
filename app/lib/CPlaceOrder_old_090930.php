	...
	if (eregi("pac$", $row->artnr)) {
		...
		*/
		...
		if (eregi("presentkort", $artnr)) {
		  ...
		  if (eregi("frakt", $artnr)  || (eregi("presentkort", $artnr))  || (eregi("invoicefee", $artnr))) {
			$bokad = 1;
		  } else {
				...
				*/
				...
				//$insert .= "values ($newordernr, '$row->artnr', $newCount, $utpris, $bokad, '$beskrivning', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $momssats, $row->momskod, '$art') ";							  	  	
		  		if ($discount) {
					  ...
					  $insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod, utprisNormal, currency) ";
					  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning $extraBeskrivningText', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $row->momssats, $row->momskod, $utprisNormal, '$curr') ";
				}
...
function process_orderrowslight($show) {
	...
	$itemsselect .= "tillverkare, lagersaldo, lagersaldo_fi, bestallt, bestallt_fi, releaseDate as rel, lev_datum_fi, lev_datum, bestallningsgrans, bestallningsgrans_fi, lev_datum_norm, lev_datum_norm_fi  ";
	//if ($fi) {	
...
function process_orderrows($show) {
	...
	//	$itemsselect  = "SELECT Orderposter_fi.artnr, Orderposter_fi.utpris, Orderposter_fi.levDatum, Orderposter_fi.bokad, Orderposter_fi.antal, Moms.momssats_fi as momssats, Orderposter_fi.beskrivning,   ";
	//else 
		$itemsselect  = "SELECT Orderposter.artnr, Orderposter.utpris, Orderposter.levDatum, Orderposter.bokad, Orderposter.rest, Orderposter.antal, Moms.momssats, Moms.momssats_fi, Orderposter.beskrivning, ";
	
	$itemsselect .= "tillverkare, lagersaldo, lagersaldo_fi, bestallt, bestallt_fi, releaseDate as rel, lev_datum_fi, lev_datum, bestallningsgrans, bestallningsgrans_fi, lev_datum_norm, lev_datum_norm_fi  ";
	//if ($fi) {	
	//	$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter_fi, Tillverkare, Moms WHERE Orderposter_fi.artnr = Artiklar.artnr AND Orderposter_fi.ordernr = $ordernrladdaom AND (Orderposter_fi.artnr not like 'frakt%') ";
	//} else {
		$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter, Tillverkare, Moms WHERE Orderposter.artnr = Artiklar.artnr AND Orderposter.ordernr = $ordernrladdaom AND NOT (Orderposter.artnr like 'frakt%' OR Orderposter.artnr = 'invoicefee') ";
	//}
	$itemsselect .= " AND Artiklar.artnr = Artiklar_fi.artnr_fi AND Artiklar.tillverkar_id = Tillverkare.tillverkar_id AND Moms.moms_id = Artiklar.momskod ";
	//echo $itemsselect;
	/**
...
<tr>
  <td height="15"></td>
  <td colspan="3" height="15"><p align="right"><font face="Verdana, Arial" size="1"> inkl moms:</font>
  </td><td bgcolor="#ECECE6" height="15"><font face="Verdana, Arial"><b><font size="1">$summaMedMoms</font></b></font></td>
</tr>
EOF;

if ($orderrow->betalsatt_id == 13) {
$messageHtml .= <<<EOF
<tr>
  <td height="15"><font face="Verdana, Arial" size="1">&nbsp;</font></td>
  <td colspan="3" height="15"><p align="right"><font face="Verdana, Arial" size="1">&nbsp;</font></td>
  <td height="15"><font face="Verdana, Arial" size="1">&nbsp;</font></td>
</tr>
<tr>
  <td height="15"><font face="Verdana, Arial" size="1">&nbsp;</font></td>
  <td colspan="3" height="15"><p align="right"><font face="Verdana, Arial" size="1"> Faktura avgift:</font></td>
  <td height="15"><font face="Verdana, Arial" size="1"><b>29 SEK</b> (avgiften tas ut direkt av Kreditor Europe)</font></td>
</tr>
EOF;
}
