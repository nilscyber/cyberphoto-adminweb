...
?>
	...
	%>
		<tr>
		<td bgcolor="#ECECE6" colspan="6"><font color="#2B2B2B"  face="Verdana,Arial"><small><small><b>
        <%
               if ($article->kategori <> ".") {
                        if ($headline) {
                             print "$headline";
	...
	<%
	 ...
	 if ($article->no_buy == 0)
	 {
			...
			print "<IMG SRC=\"/pic/kop2006c.gif\" border=0>";
			print "</A>";
		}
		else
		{
		...
		if ($article->link <> "")
			{
			 ...
			 if (eregi (".jpg$", $article->link) || eregi (".gif$", $article->link) || eregi (".jpeg$", $article->link))
			 	...
			 	print "<IMG SRC=\"/pic/info2006.gif\" border=0 >" ;
	...
	<%
		if ($article->lagersaldo > 0 )
			{
			 	print "<IMG SRC=\"/pic/lager2006b.gif\" border=0 >" ;
			}
			
		 elseif ($article->bestallningsgrans == 0)
			{
				print "<IMG SRC=\"/pic/lager2006a.gif\" border=0 >";
			}
		 else
			{
				print "<IMG SRC=\"/pic/lager2006c.gif\" border=0 >";
			}
