...
Class CAjax {
	var $conn_ms; var $conn_my; var $conn_fi; var $conn_my2;
...
function getPrinters() {
	...
	$select .= "WHERE Artiklar.kategori_id = 2 AND ej_med = 0 ";
...
function getPrinterModell($q) {

	// echo "jepp";

	echo "<select name=\"modell\" id=\"modell\" onChange=\"showPrinterModell()\">";
	echo "<option></option>";
	
	$select = "SELECT CONCAT(Tillverkare.tillverkare,' ',Artiklar.beskrivning) AS PrinterModell, Artiklar.artnr ";
	$select .= "FROM Artiklar ";
	$select .= "LEFT JOIN Tillverkare ON Artiklar.tillverkar_id = Tillverkare.tillverkar_id ";
	$select .= "WHERE Artiklar.kategori_id IN (2,215,236,414,415,449,450,451) AND ej_med = 0 AND demo = 0 AND Artiklar.tillverkar_id = '" . $q . "' ";
...
function showFaqListSearch($q,$sv,$fi) {
	...
	if ($row->prio == 1) {
		echo "<li><a class=\"toplistheading\" href=\"$link?ID=" . $row->cntr ."&question=" . $q . "&show=" . $row->kategori_id . "#area" . $row->kategori_id . "\">" . $row->q . "</a></li>\n";
	} else {
		echo "<li><a class=\"toplistheading\" href=\"$link?ID=" . $row->cntr ."&question=" . $q . "&show=" . $row->kategori_id . "#area" . $row->kategori_id . "\">" . $row->q . "</a></li>\n";
	}
