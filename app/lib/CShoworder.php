...
function getKollinrOnline ($ordernr, $land_id) {
	...
	if ($land_id == 46) {
		...
		if (mysqli_num_rows($res) > 0) {
			...
			if ($fi == 'yes') 
				$ret .= "<a href=\"kollinr.php?ordernr_check=" . $ordernr . "\">" . $row->kollinr . "</a>" . " (Klikkaamalla tästä voit seurata pakettia)";
			else
				$ret .= "<a href=\"kollinr.php?ordernr_check=" . $ordernr . "\">" . $row->kollinr . "</a>" . " (klicka för att följa paketet)";
