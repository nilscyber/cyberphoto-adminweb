...
//echo "månadskostnad: " .$svea->get_monthly_cost(3290);
//$svea->reserve_amount();
//$svea->svea_example();
//print_r($svea);
...
Class CSvea {
    ...
    function create_rows() {
		...
		if ($_SESSION['CYBAIRGUN_XBOX']) {
            $invoiceRows = array_merge($invoiceRows, write_toOrderposter("17561", $newordernr, 1, true, true));
		}
		...
		if ($_SERVER['REMOTE_ADDR'] == "192.168.1.98xx") {
            echo "<br>goodsvalueMoms före " . $goodsvalueMoms;
        }
    ...
    function reserve_amount() {
        ...
        try {
            ...
            Log::createTicketAndLog(Log::RECIPIENT_IT, $errMess, "Krasch när kund försöker skapa svea betalning", 
        ...
        if ($this->kreditor_status != 0) {
            ...
            $insert = "INSERT INTO cyberphoto.logWeb (logDate, logPage, logComment, logBasket, logDeny, logReason, logIP, logFI) values (now(), 'svea', 'Personnr:  " . $this->cust_pno .
                    ". Kundnummer: $kundnrsave. Ordersumma: $goodsvalueMoms. pclass: $this->pclass', '$kundvagn_local', '1', '$SveaErrorResult', '$logIP', '$fi')";
            ...
            $insert = "INSERT INTO cyberphoto.logWeb (logDate, logPage, logComment, logBasket, logIP, logFI) values (now(), 'svea', 'resultat: " . $this->SveaOrderId . ". Personnr:  " . $this->cust_pno . ". kundnummer: $kundnrsave. ordersumma: $goodsvalueMoms. pclass: $this->pclass', '$kundvagn_local', '$logIP', '$fi')";
			if ($this->pclass == -3) { // sätt pclass för att maila ut till berörd
