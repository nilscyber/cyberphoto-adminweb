...
class rss_feed {
	 ...
	 * @param string $xmlns
	 *        	XML namespace
	 * @param array $a_channel
	 *        	channel properties
	 * @param string $site_url
	 *        	the URL of your site
	 * @param string $site_name
	 *        	the name of your site
	 * @param bool $full_feed
	 *        	flag for full feed (all topic content)
	 */
	...
	public function get_feed_items($rss_date, $rss_items_count = 10) {
		...
		$a_topic_ids = array ();
		$sql = 'SELECT id FROM topics ' . 'WHERE date_published <= ' . "'" . $conn->real_escape_string ( $rss_date ) . "'" . 'AND date_published IS NOT NULL ' . 'ORDER BY date_published DESC ' . 'LIMIT 0,' . $rss_items_count;
