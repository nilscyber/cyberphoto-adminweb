...
function write_toOrderposter($artnr, $newordernr, $count, $includedArticle = false) {
	...
	$discountCode, $discountCodeStatus, $rowDiscount, $freight, $pay, $avtalsRabattTot, $old_avtalskund;
	...
	if (eregi("pac$", $row->artnr)) {
		...
		while ($row = mssql_fetch_object($res)):
		    ...
		    */
		    ...
		    if ($i == $antalRader) {
		    		$delAvtalsRabatt = 0;
		    		if ($old_avtalskund != "") {
		    			$delAvtalsRabatt = $bask->getContractDiscountPart($old_avtalskund, $row->artnr)*($utpris - $Totalt);
		    			$avtalsRabattTot += $delAvtalsRabatt;
		    			//echo "<br>här: " . $avtalsRabattTot;
		    		}
				//if ($fi) 
				//	$s = "UPDATE Orderposter_fi set paketRabatt = " . $Totalt . " + paketRabatt WHERE ordernr = " . $newordernr . " AND artnr = '" . $row->artnr . "'";								
				 //else 
					$s = "UPDATE Orderposter set paketRabatt = " . $Totalt . " + paketRabatt, avtalsRabatt = " . $delAvtalsRabatt . " WHERE ordernr = " . $newordernr . " AND artnr = '" . $row->artnr . "'";																			    	
				//echo "här: " . $utpris . "," . $row->antal . "," . $Totalt . "," . $momssats;
				if ($row->momskod == 1)					
					$moms1 +=  round((($utpris * $antal * $count - $Totalt - $delAvtalsRabatt) * $momssats),2); 
				if ($row->momskod == 2)
					$moms2 +=  round((($utpris * $antal * $count - $Totalt - $delAvtalsRabatt) * $momssats),2); 
				if ($row->momskod == 3)
					$moms3 +=  round((($utpris * $antal * $count - $Totalt - $delAvtalsRabatt) * $momssats),2); 
				if ($row->momskod == 4)
					$moms4 +=  round((($utpris * $antal * $count - $Totalt - $delAvtalsRabatt) * $momssats),2); 
				

		    } else {
		    		$delAvtalsRabatt = 0;
		    		if ($old_avtalskund != "") {
		    			$delAvtalsRabatt = $bask->getContractDiscountPart($old_avtalskund, $row->artnr)*($utpris - $andelKr);
		    			$avtalsRabattTot += $delAvtalsRabatt;
		    			echo "<br>här: " . $avtalsRabattTot;
		    		}		    
				//if ($fi)
		    	...
		    	//else 
		    		$s = "UPDATE Orderposter set paketRabatt = " . $andelKr . " + paketRabatt, avtalsRabatt = " . $delAvtalsRabatt . " WHERE ordernr = " . $newordernr . " AND artnr = '" . $row->artnr . "'";
				if ($row->momskod == 1)
					$moms1 +=  round((($utpris * $antal * $count - ($andelKr - $delAvtalsRabatt)) * $momssats),2);
				if ($row->momskod == 2)
					$moms2 +=  round((($utpris * $antal * $count - $andelKr - $delAvtalsRabatt) * $momssats),2); 
				if ($row->momskod == 3)
					$moms3 +=  round((($utpris * $antal * $count - $andelKr - $delAvtalsRabatt) * $momssats),2);
				if ($row->momskod == 4)
					$moms4 +=  round((($utpris * $antal * $count - $andelKr - $delAvtalsRabatt) * $momssats),2); 
		...
		*/
		...
		$delAvtalsRabatt = 0;
		if ($old_avtalskund != "") {	
			//echo "<br>här: " . $delAvtalsRabatt . " " . $artnr . " " . $utpris;
			$delAvtalsRabatt = $bask->getContractDiscountPart($old_avtalskund, $artnr)*($utpris);
			$avtalsRabattTot += $delAvtalsRabatt;
			//echo "<br>här: " . $delAvtalsRabatt . " . " . $avtalsRabattTot . " . " . $artnr . " . " . $utpris;
		}
		// Om den existerade, plussa ihop och uppdatera antalet till det nya
		if (mssql_num_rows ($res3) > 0) {
			...
			//else 
				$update2 = "UPDATE Orderposter set antal = $insertCount2, avtalsRabatt = avtalsrabatt + $delAvtalsRabatt WHERE ordernr = $newordernr AND artnr = '$artnr' ";
		//echo $update2;
		...
		// Stoppa annars in som vanligt
		  ...
		  //} else {
				...
				*/
				...
				//$insert .= "values ($newordernr, '$row->artnr', $newCount, $utpris, $bokad, '$beskrivning', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $momssats, $row->momskod, '$art') ";							  	  	
		  		if ($discount) {
					  $insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, levDatum, levDatumDatumInlagt, momssats, momskod, discountCode, utprisNormal, avtalsRabatt) ";
					  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning $extraBeskrivningText', '$levDatum', '$inkommet', $row->momssats, $row->momskod, '$discountCode', $utprisNormal, $delAvtalsRabatt) ";		  			  			
				} else {
					  $insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod, utprisNormal, avtalsRabatt) ";
					  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning $extraBeskrivningText', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $row->momssats, $row->momskod, $utprisNormal, $delAvtalsRabatt) ";
				}
		...
		if ($includedArticle) {
			...
			// Titta om "rabatt ord" redan är inlagda, och lägg till text utefter det.
			if (eregi("Övrig rabatt avser", $oldkommentar)) {

				$newkommentar = $oldkommentar . ", samt $name: " . $rabattTxt;
			}
			else 	{

				$newkommentar = "" . $oldkommentar . "Rabatten avser att $name är inluderad på köpet: " . $rabattTxt;
				//$newkommentar .= sprintf("%.2f", $ny_rabatt) . " ";
			}
			...
			*/
			...
			if ($row->momskod == 1)
				$moms1 += ($momssats*($utpris - $delAvtalsRabatt)*$count);
			elseif ($row->momskod == 2)
				$moms2 += ($momssats*($utpris - $delAvtalsRabatt)*$count);
			elseif ($row->momskod == 3)
				$moms3 += ($momssats*($utpris - $delAvtalsRabatt)*$count);
			elseif ($row->momskod == 4)
				$moms4 += ($momssats*($utpris - $delAvtalsRabatt)*$count);		
		}
...
<tr>
  ...
  <td colspan="4" height="15">
    <p align="right"><font face="Verdana, Arial" size="1">Faktureringsavgift med 29 kr tillkommer&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font>
  </td>
</tr>
EOF;
}
