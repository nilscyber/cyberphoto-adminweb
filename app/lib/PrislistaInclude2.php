...
$select  = "SELECT tillverkare, Artiklar.beskrivning, Artiklar.artnr, Artiklar.manufacturerproductno, utpris, kategori, link, lagersaldo, bild, Moms.momssats, Artiklar.WebshopURL, Artiklar.WebshopImageURL, Artiklar.WebshopCategory, Artiklar.upc FROM  ";
...
// $select .= " AND NOT (Artiklar.kategori_id = 486 OR Artiklar.kategori_id = 501 OR Artiklar.kategori_id = 509) ";
$select .= " AND NOT (Artiklar.kategori_id IN (396,486,501,509,513)) ";
$select .= "AND NOT (Artiklar.kategori_id IN (1000260)) " ;
$select .= " AND Artiklar.WebshopURL != ''";
...
function extr($row) {
	...
	$linken = "http://www.cyberphoto.se/" . $linken . "&utm_source=pricerunner&utm_medium=cpc&utm_content=" . $row->artnr . "&utm_campaign=pricerunner_se";
