...
function createArticleSelect2() {
	return("SELECT * FROM (SELECT artnr, Artiklar.beskrivning, Artiklar.kommentar, utpris, bild, tillverkare, link, link_fi, kategori, kategori_fi, ej_med, no_buy, motljsk, filterd, skick, garanti, lagersaldo, filmkat_id, releaseDate as rel, zoom, ccd, betyg, zoom_digikam, Moms.momssats, bestallningsgrans ".
...
function readArticlesWithCriteria2($criteria) {
	...
	$sel = createArticleSelect2()." $criteria";	
	//echo "<p>" . $sel . "<p>";
	//exit;
	...
	while ($conn->next_record()) {
		...
		$article = new CArticle2($conn->Record);
