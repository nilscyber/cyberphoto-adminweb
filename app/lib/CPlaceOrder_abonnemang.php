...
function write_toOrderposter($artnr, $newordernr, $count, $includedArticle = false) {
	...
	$select .= "bestallt, bestallt_fi, lev_datum, lev_datum_fi, lev_datum_norm, lev_datum_norm_fi, bestallningsgrans, bestallningsgrans_fi, releaseDate as rel  ";
	...
	if ($row->kategori_id == 595 || $row->kategori_id == 630) {
		$isHidden = -1;
		// $bokad = 1;
	}
	if ($row->kategori_id == 595 || $row->kategori_id == 629 || $row->kategori_id == 630) {
		// $isHidden = -1;
		$bokad = 1;
	}

	if ($row->kategori_id == 595 && $_SESSION['MOB_PRICE'] == 1) {
		$outprice = $_SESSION['MOB_DIFF_COUNT'];
	}

	if ($_SESSION['MOB_PHONE'] == $artnr) {
		$beskrivning_alt = $_SESSION['MOB_BESKRIVNING'];
		$visualPrice = ($_SESSION['MOB_PRICE'] * 0.8);
	}
	...
	if ($discountCode != "" && $discountCodeStatus == 1 && $rowDiscount->totalSum == "") {							
	...
	if (eregi("pac$", $row->artnr)) {
			...
			$select .= "frakt, lagersaldo_fi, bestallt_fi, Artiklar.releaseDate as rel, Artiklar.utpris, Artiklar.till_kund, Artiklar_fi.till_kund_fi, ";
		...
		*/
		...
		while ($row = mssql_fetch_object($res)):
			...
			if ($row->artnr == "63608367" && $fi && !$sv) {				
				$select2  = "SELECT Artiklar.artnr, Artiklar.lagersaldo, Artiklar.beskrivning, Artiklar.bestallt, beskrivning_fi, Artiklar.kommentar, Artiklar_fi.kommentar_fi, utpris_fi, tillverkare, ";
				$select2 .= "frakt, lagersaldo_fi, bestallt_fi, Artiklar.releaseDate as rel, Artiklar.utpris, Artiklar.till_kund, Artiklar_fi.till_kund_fi, ";
				$select2 .= "lev_datum_fi, bestallningsgrans, lev_datum_norm, lev_datum_norm_fi, Artiklar.ej_brev as ej_brev, Kategori.ej_brev as kat_ej_brev, Artiklar.momskod, ";
				$select2 .= "Moms.momssats_fi, Moms.momssats, Artiklar.link, link2_fi, art_id, art_id_fi ";
				...
				$res3 = mssql_query($select2);
				$row = mssql_fetch_object($res3);
			}
			$frakt = $row->frakt;
			...
			*/
			...
			if ($fi) {
				...
				$extraCnt = $bask->check_queue($row->artnr, $fi, null);
				if ($row->bestallt_fi >= $count + $extraCnt) // om sant så letar vi bara igenom finska beställningstabellen. 
					$bask->kollaLevTid($row->artnr, $count , null, true, $fi, $sv);
				else 
					$bask->kollaLevTid($row->artnr, $count, null, false, $fi, $sv);
			} else 	{
			*/
				$bask->kollaLevTid($row->artnr, $newCount, null, false, $fi, $sv);				
			//}
			if ($bask->prec == "")
				$bask->prec = 0;
			...
			if (mssql_num_rows ($res2) > 0) {
				...
				//if ($fi)
				//	$update = "update Orderposter_fi    set antal = '$insertCount', lastDeliveryDate = '" . $bask->ber_lev_datum . "', prec = " . $bask->prec . " WHERE ordernr = " . $newordernr  . " AND artnr = '$row->artnr' ";
				//else 
					$update = "update Orderposter    set antal = '$insertCount', lastDeliveryDate = '" . $bask->ber_lev_datum . "', prec = " . $bask->prec . " WHERE ordernr = " . $newordernr  . " AND artnr = '$row->artnr' ";
				mssql_query ($update, $conn_standard);
				//echo $update;
			 }
			// Stoppa annars in som vanligt
				...
				//if ($fi) {
				//	$insert  = "insert into Orderposter_fi (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod, paketArtnr) ";
				//	$insert .= "values ($newordernr, '$row->artnr', $newCount, $utpris, $bokad, '$beskrivning', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $momssats, $row->momskod, '$art') ";				
				//} else {
					$insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod, paketArtnr, currency) ";
					$insert .= "values ($newordernr, '$row->artnr', $newCount, $utpris, $bokad, '$beskrivning', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $momssats, $row->momskod, '$art', '$curr') ";				
				//}
				//echo $insert;
		...
		while ($row = mssql_fetch_object($res)):
			...
			if ($row->artnr == "63608367" && $fi && !$sv) {				
				...
				$select .= "frakt, lagersaldo_fi, bestallt_fi, Artiklar.releaseDate as rel, Artiklar.utpris, Artiklar.till_kund, Artiklar_fi.till_kund_fi, ";
				$select .= "lev_datum_fi, bestallningsgrans, lev_datum_norm, lev_datum_norm_fi, Artiklar.ej_brev as ej_brev, Kategori.ej_brev as kat_ej_brev, Artiklar.momskod, ";
				$select .= "Moms.momssats_fi, Moms.momssats, Artiklar.link, link2_fi, art_id, art_id_fi ";
				$select .= " FROM Artiklar, Artiklar_fi, Tillverkare, Kategori, Moms ";
				$select .= "WHERE Artiklar.artnr = Artiklar_fi.artnr_fi AND Artiklar.tillverkar_id = Tillverkare.tillverkar_id AND Artiklar.kategori_id = Kategori.kategori_id AND Artiklar.momskod = Moms.moms_id ";
				$select .= " AND Artiklar.artnr = '63608367_fi'";
				$res3 = mssql_query($select);
				$row = mssql_fetch_object($res3);
			}
		    ...
		    */
		    ...
		    if ($i == $antalRader) {
				...
				if ($row->momskod == 1)					
					$moms1 +=  round((($utpris * $antal * $count - $Totalt) * $momssats),2); 
				if ($row->momskod == 2)
					$moms2 +=  round((($utpris * $antal * $count - $Totalt) * $momssats),2); 
				if ($row->momskod == 3)
					$moms3 +=  round((($utpris * $antal * $count - $Totalt) * $momssats),2); 
				if ($row->momskod == 4)
					$moms4 +=  round((($utpris * $antal * $count - $Totalt) * $momssats),2); 
				...
				if ($row->momskod == 1)
					$moms1 +=  round((($utpris * $antal * $count - $andelKr) * $momssats),2);
				if ($row->momskod == 2)
					$moms2 +=  round((($utpris * $antal * $count - $andelKr) * $momssats),2); 
				if ($row->momskod == 3)
					$moms3 +=  round((($utpris * $antal * $count - $andelKr) * $momssats),2);
				if ($row->momskod == 4)
					$moms4 +=  round((($utpris * $antal * $count - $andelKr) * $momssats),2); 
		...
		*/
		...
		if (eregi("presentkort", $artnr)) {
		  ...
		  if (eregi("frakt", $artnr) || (eregi("presentkort", $artnr)) || (eregi("invoicefee", $artnr)) || $row->kategori_id == 595 || $row->kategori_id == 629 || $row->kategori_id == 630) {
			$bokad = 1;
		  } else {
		 ...
		 //$this->kollaLevtid($row->artnr, $row->antal, $row->ordernr, false, $fi, $sv )
		/**
		if ($fi) {
			$extraCnt = $bask->check_queue($row->artnr, $fi, null);
			if ($row->bestallt_fi >= $count + $extraCnt)
				$bask->kollaLevTid($row->artnr, $count, null, true, $fi, $sv);
			else 
				$bask->kollaLevTid($row->artnr, $count, null, false, $fi, $sv);
		} else 	{
		*/
		...
		// Stoppa annars in som vanligt
		  ...
		  //if ($fi) {
		  //	  $insert  = "insert into Orderposter_fi (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod) ";
		  //	  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $row->momssats_fi, $row->momskod) ";		  	
		  //} else {
			  	/**
				...
				*/
				//$insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod, paketArtnr) ";
				//$insert .= "values ($newordernr, '$row->artnr', $newCount, $utpris, $bokad, '$beskrivning', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $momssats, $row->momskod, '$art') ";							  	  	
		  		if ($discount) {
					  $insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, levDatum, levDatumDatumInlagt, momssats, momskod, discountCode, utprisNormal, currency) ";
					  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning $extraBeskrivningText', '$levDatum', '$inkommet', $row->momssats, $row->momskod, '$discountCode', $utprisNormal, '$curr') ";		  			  			
				} else {
					  $insert  = "insert into Orderposter (ordernr, artnr, antal, utpris, bokad, beskrivning, lastDeliveryDate, prec, levDatumDatumInlagt, momssats, momskod, utprisNormal, currency, beskrivning_alt, isHidden, visualPrice) ";
					  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning $extraBeskrivningText', '$bask->ber_lev_datum', $bask->prec, '$inkommet', $row->momssats, $row->momskod, $utprisNormal, '$curr', '$beskrivning_alt', '$isHidden', '$visualPrice') ";
				}
		...
		if ($includedArticle) {
			...
			$rabattTxt = number_format($ny_rabatt, 0, ',', ' ') . " " . $val;
			...
			*/
			...
			//} else {
				if ($newkommentar != "")
					$insert_rabatt = "UPDATE Ordertabell SET rabatt = rabatt + " . $ny_rabatt . ", kommentar = '" . $newkommentar . "'" . " WHERE ordernr = $newordernr";
				else 
					$insert_rabatt = "UPDATE Ordertabell SET rabatt = rabatt + " . $ny_rabatt . " WHERE ordernr = $newordernr";
			//}
			mssql_query ($insert_rabatt, $conn_standard);
...
function process_orderrowslight($show) {
	...
	$est_delivery, $fi, $sv, $fi, $sv, $bask, $conn_standard;
	...
	//else 
		$itemsselect  = "SELECT Orderposter.artnr, Orderposter.utpris, Orderposter.levDatum, Orderposter.bokad, Orderposter.antal, Moms.momssats, Moms.momssats_fi, Orderposter.beskrivning, Orderposter.beskrivning_alt, Orderposter.visualPrice, ";
	...
	//} else {
		$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter, Tillverkare, Moms WHERE Orderposter.artnr = Artiklar.artnr AND Orderposter.ordernr = $ordernrladdaom AND (Orderposter.artnr not like 'frakt%') AND NOT (isHidden = -1)  ";
	//}
	...
	*/
	...
	while ($row = mssql_fetch_object($itemres)) :		
		...
		*/
		...
		if ($row->tillverkare != '.' && strlen($beskrivning_alt) < 2) 
			$benamning = $row->tillverkare . " " . $benamning;
		
		if ($show) {
		?>
			<tr>
			<td><font face="Verdana, Arial" size="1"><?php echo $count; ?> st</font></td>
			<td><font face="Verdana, Arial" size="1"><?php echo $benamning; ?></font></td>
			<?php if ($row->visualPrice > 0) { ?>
			<td align=right><font face="Verdana, Arial" size="1"><?php echo number_format(($row->visualPrice + $row->visualPrice * $momssats) * $row->antal, 0, ',', ' ') . " " . $val; ?></font></td>
			<?php } elseif ($row->kategori_id == 629) { ?>
			<td align=right><font face="Verdana, Arial" size="1">&nbsp;</font></td>
			<?php } else { ?>
			<td align=right><font face="Verdana, Arial" size="1"><?php echo number_format(($row->utpris + $row->utpris * $momssats) * $row->antal, 0, ',', ' ') . " " . $val; ?></font></td>
			<?php } ?>
			</tr>
			<?php
		}			
	...
	//	$itemsselect  = "SELECT Orderposter_fi.artnr, Orderposter_fi.utpris, Orderposter_fi.levDatum, Orderposter_fi.bokad, Orderposter_fi.antal, Moms.momssats_fi as momssats, Orderposter_fi.beskrivning ";
	//else 
		$itemsselect  = "SELECT Orderposter.artnr, Orderposter.utpris, Orderposter.levDatum, Orderposter.bokad, Orderposter.antal, Moms.momssats, Moms.momssats_fi, Orderposter.beskrivning ";
	...
	while ($row = mssql_fetch_object($itemres)) :
		...
		?>
			...
			<tr>
			<td><font face="Verdana, Arial" size="1">1 st</font></td>
			<td><font face="Verdana, Arial" size="1"><?php echo $benamning; ?></font></td>
			<td align=right><font face="Verdana, Arial" size="1"><?php echo number_format(($row->utpris + $row->utpris * $momssats) * $row->antal, 0, ',', ' ') . " " . $val; ?></font></td>
			</tr>
	...
	endwhile;
		...
		<?php if ($show) { ?>
		<tr>
		  <td><font face="Verdana, Arial" size="1"><% echo " "; %></font></td>
		  <td><font face="Verdana, Arial" size="1"><% echo $Paketrabatt; %></font></td>
...
function process_orderrows($show) {
	...
	//	$itemsselect  = "SELECT Orderposter_fi.artnr, Orderposter_fi.utpris, Orderposter_fi.levDatum, Orderposter_fi.bokad, Orderposter_fi.antal, Moms.momssats_fi as momssats, Orderposter_fi.beskrivning,   ";
	//else 
		$itemsselect  = "SELECT Orderposter.artnr, Orderposter.utpris, Orderposter.levDatum, Orderposter.bokad, Orderposter.rest, Orderposter.antal, Moms.momssats, Moms.momssats_fi, Orderposter.beskrivning, Orderposter.beskrivning_alt, Orderposter.visualPrice, ";
	
	$itemsselect .= "tillverkare, lagersaldo, lagersaldo_fi, bestallt, bestallt_fi, releaseDate as rel, lev_datum_fi, lev_datum, bestallningsgrans, bestallningsgrans_fi, lev_datum_norm, lev_datum_norm_fi, kategori_id  ";
	//if ($fi) {	
	//	$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter_fi, Tillverkare, Moms WHERE Orderposter_fi.artnr = Artiklar.artnr AND Orderposter_fi.ordernr = $ordernrladdaom AND (Orderposter_fi.artnr not like 'frakt%') ";
	//} else {
		$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter, Tillverkare, Moms WHERE Orderposter.artnr = Artiklar.artnr AND Orderposter.ordernr = $ordernrladdaom AND NOT (Orderposter.artnr like 'frakt%' OR Orderposter.artnr = 'invoicefee') AND NOT (isHidden = -1) ";
	//}
	$itemsselect .= " AND Artiklar.artnr = Artiklar_fi.artnr_fi AND Artiklar.tillverkar_id = Tillverkare.tillverkar_id AND Moms.moms_id = Artiklar.momskod ";
	//echo $itemsselect;
	/**
	...
	*/
	...
	//	$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter_fi, Tillverkare, Moms WHERE Orderposter_fi.artnr = Artiklar.artnr AND Orderposter_fi.ordernr = $ordernrladdaom AND (Orderposter_fi.artnr like 'frakt%') ";
	//} else {
		$itemsselect .= " FROM Artiklar, Artiklar_fi, Orderposter, Tillverkare, Moms WHERE Orderposter.artnr = Artiklar.artnr AND Orderposter.ordernr = $ordernrladdaom AND (Orderposter.artnr like 'frakt%' OR Orderposter.artnr = 'invoicefee') ";
	//}
	$itemsselect .= " AND Artiklar.artnr = Artiklar_fi.artnr_fi AND Artiklar.tillverkar_id = Tillverkare.tillverkar_id AND Moms.moms_id = Artiklar.momskod ";	
	$itemres = mssql_query($itemsselect, $conn_standard);
...
$messageHtml .= <<<EOF
<tr><td bgcolor="#ECECE6" colspan="4" height="15"><font face="Verdana, Arial" size="1">&nbsp;&nbsp;&nbsp;
...
<tr>
  <td height="15"></td>
  <td colspan="2" height="15"><p align="right"><font face="Verdana, Arial" size="1"> inkl moms:</font>
  </td><td bgcolor="#ECECE6" height="15" align="right"><font face="Verdana, Arial"><b><font size="1">$summaMedMoms</font></b></font></td>
</tr>
EOF;
