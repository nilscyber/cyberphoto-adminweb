...
?>
	...
	%>
		...
		<tr>
		<td colspan="4"><font color="#2B2B2B"  face="Verdana,Arial"><small><small><b>
        <%
               if ($article->kategori <> ".") {
                        if ($headline) {
                             print "$headline";
	...
	<%
	 if ($article->no_buy == 0) {
	 	if (eregi("pac$", $article->artnr)) {
			print "<a onmouseover=\"return escape('<b>Paketlösning</b><br>Denna artikel innehåller flera artiklar. Klicka på produkten eller infoknappen för aktuell lagerstatus.')\"><img src=\"/pic/parcel.gif\" border=0></a>&nbsp;";
	 		}
	 	if ($article->lagersaldo > 0 || eregi("pac$", $article->artnr)) {
