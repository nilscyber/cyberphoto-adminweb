...
<%		if (!(eregi("^frakt", $artnr))): %>
		...
		<% if ($fi == 'yes'): %>muuta lukumäärä</font></A>
		<% else: %>
		<img src="../pic/besantal.gif" width=62 height=8 border=0 alt="tryck för att ändra antal"></font></A>
		<% endif; %>
		<% else: %>
		&nbsp;&nbsp;
		<% endif; %>
		...
		if ($lagersaldo >= $count || ($package_stock == '1') )  { 
			...
			if ($bestallt >= $neededStock) {
				if ($lev_datum == '-' || ($lev_datum == ""))
					{ 
						if ($fi == 'yes')	
							print "toimituspäivämäärä ei ole määritelty"; 
						else
							print "ej fastställt";
					}
				else
					{ 
						if ($fi == 'yes')
							print "arvioitu saapumisaika $lev_datum"; 
						else
				...
				if (ereg("pac$",$artnr))
					{  
						...
						if ($fi == 'yes')
							print "<a href=\"javascript:levStatusPaket('$artnr', '$count') \">Lisää tietoja tästä</a>";
						else
							print "<a href=\"javascript:levStatusPaket('$artnr', '$count') \">Klicka för info</a>";
					}
				elseif ($bestallningsgrans == '0') 
					{  
						if ($fi == 'yes')
							print "tilaustuote, normaali toimitusaika $lev_datum_normal";
						else
					...
					{ 
						if ($fi == 'yes')
							print "tilapäisesti loppunut, normaali toimitusaika $lev_datum_normal" ;
						else
