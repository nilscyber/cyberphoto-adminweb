...
function write_toOrderposter($artnr, $newordernr, $count) {
	...
	$select  = "SELECT Artiklar.artnr, Artiklar.beskrivning, Artiklar_fi.beskrivning_fi, Artiklar.momskod, utpris, utpris_fi, frakt, lagersaldo, lagersaldo_fi, till_kund, till_kund_fi, Moms.momssats, Moms.momssats_fi, "; 
	...
	if ($fi) {	
		$goodsvalue+=($row->utpris_fi*$count);
		$outprice = $row->utpris_fi;	
	} else  {
		$goodsvalue+=($row->utpris*$count);
		$outprice = $row->utpris;
	}
		...
		*/
		...
		// Stoppa annars in som vanligt
		  ...
		  if ($fi) {
			  ...
			  $insert .= "values ($newordernr, '$artnr', $count, $utpris, $bokad, '$beskrivning', '$levDatum', '$inkommet', $row->momssats, $row->momskod) ";		  	
		  }
		//echo $insert;
			if ($artnr != "")
				mssql_query ($insert, $conn_standard);
