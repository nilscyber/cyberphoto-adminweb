...
# version	2000-11-20
	...
	if (count($articles) < 1) {
		$headline = "Det finns ingen kamera som matchar dina önskemål. Ändra i fälten ovan."; // fast med röd text,, 
	}
	else {
		$headline = "Digitalkameror - Utifrån Dina önskemål ovan! ";
	}
	...
	<?php if (count($articles) < 1) { ?>
		<td colspan="4"><font color="#85000D" face="Verdana,Arial" size="1"><b>
	<?php } else { ?>
...
?>
	...
	<%
	 ...
	 if ($article->no_buy == 0) {
	 	...
	 	elseif ($article->lagersaldo_fi > 0 || $article->lagersaldo > 0 || eregi("pac$", $article->artnr)) {
