...
Class CSvea {
    ...
    function __construct($test = false) {
    	$this->test = $test;
        if ($this->test)
            $this->wsdlLink = "https://webpaywsstage.svea.com/SveaWebPay.asmx?WSDL";
        else
            $this->wsdlLink = "https://webpayws.svea.com/SveaWebPay.asmx?WSDL";
    }
    ...
    function create_rows() {
        ...
        if ($pay == "sveainvoice" && $old_foretag != -1 && $fi) // 2015-06-30 Denna tar vi bara om det �r Finland just nu, Sverige �r utan avgift
		...
		if ($_SERVER['REMOTE_ADDR'] == "192.168.1.98xx") {
            echo "<br>goodsvalueMoms f�re " . $goodsvalueMoms;
        }
    ...
    /**
     * Updated paymentplans from svea
     * @param type $no_update - if true, no actual updated is performed
     * @return array with paymentplans from Svea
     */
    function upatePaymentPlans($no_update = false) {
        ...
        foreach ($r->GetPaymentPlanParamsEuResult->CampaignCodes->CampaignCodeInfo as $key => $plan) {
            ...
            if ($plan->CampaignCode != 310000) {
				...
				$i = "REPLACE INTO cyberphoto.SveaPaymPlans (CampaignCode, Description, PaymentPlanType, ContractLengthInMonths, MonthlyAnnuityFactor, InitialFee, NotificationFee,
				...
				if (!mysqli_query(Db::getConnection(true), $i)) {
					echo "\n" . Db::getConnection(true) . "\n";
					echo "error: " . mysqli_errno(Db::getConnection(true)) . " : " . mysqli_error(Db::getConnection(true));
				}
    ...
    function set_svea_login($reload = false) {
        ...
        //*/
        if (Locs::getCountry() == Locs::COUNTRY_FI) {
        ...
        } else if (Locs::getCountry() == Locs::COUNTRY_NO) {
            if ($pay == "sveainvoice" && $old_foretag == -1) {
            ...
            if ($pay == "sveainvoice" && $old_foretag == -1) {
				...
				$this->sveaUserName = 'CYBERPHOTO'; 
				$this->sveaPassword = 'MV7arL';
                // faktura f�retag
            } else if ($pay == "sveainvoice") {
                $this->sveaAccountNo = '76222';
				$this->sveaUserName = 'CYBERPHOTOAB'; 
				$this->sveaPassword = 'z7B2Qp';	
        ...
        $s = "SELECT * FROM cyberphoto.svea_accounts WHERE accountno = " . $this->sveaAccountNo;
        //$res = mysqli_query($s);
    ...
    function reserve_amount() {
        ...
        if (strlen($_SESSION['old_telnr']) > 5) {
			//error_log("-- SVEA --- TELNR valt---");
            $phone = $_SESSION['old_telnr'];
        } else {
			//error_log("-- SVEA --- MOBILNR valt---");
        ...
        $request = Array(
            ...
            "CreateOrderInformation" => Array(
                ...
                "OrderDate" => date('c'),
                "AddressSelector" => $old_foretag == -1 ? $this->AddressSelector : null, // TODO: vs  $_SESSION['old_invoice_addresselector']  ??
        ...
        try {
            //Create soap client
            $client = new SoapClient($this->wsdlLink);
            // Call createOrderEU servie
            $r = $client->CreateOrderEu($data);
        } catch (Exception $exc) {
             $this->kreditor_status = -1;
            // om error, spara, maila etc
            $errMess = "Kundnummer: " . $kundnrsave . "\n";
            $errMess .= "error mess:\n";
            $errMess .= $exc->getTraceAsString();
            $errMess .= "\nlast request headers:\n";
            if (is_object($client))
                $errMess .= print_r($client->__getLastRequestHeaders(), true); //the headers of your last request
            $errMess .= "\nlast request:";
            if (is_object($client))
                $errMess .= print_r($client->__getLastRequest(), true); //your last request
            $errMess .= "\n end soap error handling \n";
            $errMess .= "\nAlla variabler: \n" . print_r(get_defined_vars(), true);
            Log::createTicketAndLog(Log::RECIPIENT_IT, $errMess, "Krasch n�r kund f�rs�ker skapa svea betalning",
                    Log::LEVEL_ALERT, null, null);
        }
        //if ($_SERVER['REMOTE_ADDR'] == "192.168.1.98xx") {
        	Log::addLog("Nils testar svea: \n" . print_r($data, true), Log::LEVEL_INFO);
        //}
        ...
        // TODO: and $no ?
        if ($r->CreateOrderEuResult->Accepted == 1 && Locs::getCountry() == Locs::COUNTRY_FI ) {
        ...
        if ($this->kreditor_status != 0) {
			...
			if ($this->pclass == -3) { // s�tt pclass f�r att maila ut till ber�rd
    ...
    function sendMessIfPclass($kundnrsave, $personnr, $goodsvalueMoms, $result, $pclass, $kundvagn) {
        ...
        // $text1 .= "Orsak: " . $result . "\n\n";
        if ($pclass == 213060) {
			$text1 .= "Betals�tt: K�p nu betala om tre m�nader\n\n";
		} else {
			$text1 .= "Betals�tt: " . $pclass . "\n\n";
		}
        $text1 .= "Kundvagn: " . $kundvagn . "\n\n";
    ...
    function get_address($pno) {
        ...
        if ($svea_req->GetAddressesResult->Accepted == 1) {
			...
			// sortera arrayen efter addressline1 f�rst om den finns, annars adressline2
			function cmp($as, $bs)
			{

				$c1 = trim($as->AddressLine1);

				if ($c1 == "") $c1 = $as->AddressLine2;

				$c2 = trim($bs->AddressLine1);

				if ($c2 == "") $c2 = $bs->AddressLine2;
				return strcmp($c1, $c2);
			}
			...
			//Tools::print_rw($a);
			//exit;
            // spara adresserna i detta svea-objekt f�r senare anv�ndning.
            $this->sveaInvoiceAddress = $a;

            $a = $a[0]; // quick-fix s� jag slapp �ndra nedanst�ende rader
            ...
            if ($a->AddressLine1 != "") { // c/o address
            	$r[0][2] = $a->AddressLine1;
            	$r[0][3] = $a->AddressLine2;
            } else {
            ...
            $r[0][4] = $a->Postcode;
            $r[0][5] = $a->Postarea;
            $r[0][6] = $a->AddressLine1;
            $r[0][7] = $a->AddressLine2;
            // if business
            $r[0][8] = $a->LegalName;
            /**
              $r[0][0] = ($a->FirstName);
    ...
    function get_address_raw($pno, $isCompany = false) {
        global $eid, $secret;
        $this->set_svea_login(true);
        ...
        if ($svea_req->GetAddressesResult->Accepted == 1) {
            ...
            return json_encode($svea_req->GetAddressesResult);
        }
	...
	function list_business_locations($new_invoice_address) {
        ...
        if (!isset($this->cust_addresses))
			...
			if ($new_invoice_address == "" || $new_invoice_address == "noinvoice") {
				...
				$ret .= "\t\t<option value=\"noinvoice\" selected>V�lj fakturaadress</option>\n";
			} else {
				...
				$ret .= "\t\t<option value=\"noinvoice\">V�lj fakturaadress</option>\n";
			}
	...
	function list_business_delivery_locations($new_delivery_address) {
        ...
        if (!isset($this->cust_addresses))
			...
			if ($new_delivery_address == "" || $new_delivery_address == "nodelivery") {
				...
				$ret .= "\t\t<option value=\"nodelivery\" selected>V�lj leveransadress</option>\n";
			} else {
				...
				$ret .= "\t\t<option value=\"nodelivery\">V�lj leveransadress</option>\n";
			}
    ...
    function print_addresses_new() {
        ...
        if ($this->cust_addresses[0][3]!="")
        	$ret .= "<input class='inputaddressgrey' type='text' name='new_adress' size='28' value='" . $this->cust_addresses[0][3] . "' onFocus='this.blur()'><br>";
        $ret .= "<input class='inputaddressgrey' type='text' name='new_postnr' size='5' value='" . $this->cust_addresses[0][4] . "' onFocus='this.blur()'>";
        $ret .= "<input class='inputaddressgrey' type='text' name='new_postadr' size='16' value='" . $this->cust_addresses[0][5] . "' onFocus='this.blur()'>";
    ...
    function print_addresses_mobilesite() {
        ...
        $ret .= "<input class='inputaddressgrey' type='text' name='new_adress' size='25' value='" . $this->cust_addresses[0][3] . "' onFocus='this.blur()'><br>";
        $ret .= "<input class='inputaddressgrey' type='text' name='new_postnr' size='5' value='" . $this->cust_addresses[0][4] . "' onFocus='this.blur()'>";
        $ret .= "<input class='inputaddressgrey' type='text' name='new_postadr' size='14' value='" . $this->cust_addresses[0][5] . "' onFocus='this.blur()'>";
    ...
    function periodic_cost($payment_period, $goodsValueMoms, $currency = "SEK") {
        ...
        $res = mysqli_query(Db::getConnection(), $s);
        $row = mysqli_fetch_object($res);
        return ( round($row->MonthlyAnnuityFactor * $goodsValueMoms, 0) );
    }

    function periodic_costNew($payment_code, $goodsValueMoms, $ismobile) {
        ...
        $row = mysqli_fetch_object($res);
		if ($ismobile) {
			return ( round(($row->MonthlyAnnuityFactor * $goodsValueMoms) + $row->NotificationFee, 0) );
		} else {
			return ( round($row->MonthlyAnnuityFactor * $goodsValueMoms, 0) );
		}
    }
