...
function customer_info($lastInfo) {
	...
	*/
	...
	$freight, $pay, $fi, $sv, $old_villkor_id, $old_personnr, $old_firstName, $old_lastName, $old_firstNameDel, $old_lastNameDel, $old_foretag, $old_avisera,  $old_splitOrder;
	...
	if ($confirm == 1) {
		...
		//echo "härrrr: " . $old_forsakringserbjudande;
...
function add_customer($uppdatera, $mailpassw) {
	...
	$new_avisera, $new_forsakringserbjudande, $old_forsakringserbjudande;
	...
	echo "härasf: " . $old_forsakringserbjudande . "." . $new_forsakringserbjudande;
...
function getDiscountArticles($articles, $fi) {
	...
	*/
	...
	foreach ($articles as $article)	{
		...
		//EL-26385
		$res = mssql_query($select, $this->conn_ms);
		//echo "<p>ant rad: " . mssql_num_rows($res) . "<p>";
		if (mssql_num_rows($res) > 0) {
			...
			while ($row = mssql_fetch_object($res)) {
				...
				if ( ($row->freeFreight == -1 && ($row->discountCodes.totalSum > $this->basketValue || $row->discountCodes.totalSum == 0 || $row->discountCodes.totalSum == "" ) && !$fi ) ||
					...
					//echo "här: " . $freight;
					if (eregi("^fraktpost", $freight)) { // endast om frakten är postpaket blir det fraktfritt (även mot pf)
					...
					while ($rowDiscount = mssql_fetch_object($res2)) {	
						...
						if ($f === false) {
							...
							if ($rowDiscount->articleDiscounted == -1) {      // om det är en rabatterad vara, märk upp vilken spara											     
								...
								$i = 0;
								while ($rw = mssql_fetch_array($rs)) {
									array_push($includedArtsTmp, $rw);
									echo "här:<br>";
									$this->obsafe_print_r($rw, false, true);
	...
	if (eregi("pac$", $artnr))  {
		...
		while ($row = mysqli_fetch_object($res)) {
			...
			if (mssql_num_rows($res2) > 0) {
				...
				array_push($arr, $row->descrptn);
				if (!is_array($this->includedArticles))
					$this->includedArticles = array();
				array_push($this->includedArticles, $arr);
				$beskrivning .= $row->descrptn;
				//print_r($this->includedArticles);
