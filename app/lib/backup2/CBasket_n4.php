...
function getDiscountText($rowDiscount, $momssats, $utpris, $showDecoration) {
	...
	if ($fi) {
		...
		if ($rowDiscount->discountPercent != "") {
			$txt = round($rowDiscount->discountPercent * 100, 0) .  "% rabatt avdragen enligt kupong";
		} elseif ($rowDiscount->discountAmount != "") {
			$txt = round($rowDiscount->discountAmount + $rowDiscount->discountAmount * $momssats, 0) .  " SEK rabatt avdragen enligt kupong";
		} elseif ($rowDiscount->discountOutprice != "") {
			//echo "här";
			$txt = "nytt utpris enligt kupong. Ordinarie pris: " . round($utpris + $utpris * $momssats, 0) . " SEK";
	...
	if ($txt != "") {
		if ($showDecoration)
			$txt = "<br><font color=\"#85000D\">(" . $txt . ")</font>";
		else 	
			$txt = "(" . $txt . ")";
	}
	return $txt;		
