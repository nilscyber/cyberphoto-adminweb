...
Class CBasket {
	...
	//, &$discountPercent, &$discountAmount, &$discounts
	var $discountPercent; var $discountAmount; var $discounts;
...
function getDiscountArticles($articles, $fi) {
	...
	*/
	...
	foreach ($articles as $article)	{
		...
		$select = "SELECT discountCodes.*, discountCodeArticles.artnr as artnrMatch, discountCodeArticles.articleDiscounted FROM discountCodes, discountCodeArticles 
		WHERE discountCodeArticles.dscntId = discountCodes.cnt AND discountCodeArticles.artnr = '" . $article[0] . "' AND validDate > getdate() " . $notSel;
...
function viewItemsInBasket($firstbasket, $fi) {
	...
	$PHP_SELF, $brev, $bestallningsgrans, $pack, $alltidBrev, $pallDelivery, $fi, $sv, $bestallt_fi, $lev_datum_norm_fi, $lev_datum_norm, $release, $discountCode, $discountCodeStatus, $freight, $pay, $basketValue, $articles, $discountAmount;	
		...
		*/
		...
		if ($firstbasket != 'nooutput') {
			if (count($this->discounts) > 0) {
				//echo "här: " . count($this->discounts);
				$j = 0;
				//$this->obsafe_print_r($this->discounts);
...
//			0	  1		2				3		4		5		6			   7
					...
					if (eregi("pac$", $artnr))  {
						...
						while ($row = mysqli_fetch_object($res)) { 
							if ($discount[2] == $row->artnr && !$discount[7]) {
								//$this->obsafe_print_r($discount, false, true);
								if ($discount[3] > 0) {
									$discountAmount = $discount[3]; 			
									$utpris = $utpris - $discountAmount;
									$goodsvalue -= $discountAmount;
									$goodsvalueMoms -= $discountAmount * 1.25;		
								} else if ($discount[4] > 0) {
									$discountAmount = $discount[4] * $utpris;		
									$utpris = $utpris - $discountAmount;
									$goodsvalue -= $discountAmount;
									$goodsvalueMoms -= $discountAmount * 1.25;							
								}
								$extraBeskrivningText .=  " (" . $discount[1] . ")";
								$this->discounts[$j][7] = true;
							} 	
						}
					} else {				

						if ($discount[2] == $artnr && !$discount[7]) {
							//$this->obsafe_print_r($discount, false, true);
							if ($discount[3] > 0) {
								$discountAmount = $discount[3]; 			
								$utpris = $utpris - $discountAmount;
