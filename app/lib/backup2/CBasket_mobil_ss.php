...
function add_customer($uppdatera, $mailpassw) {
	...
	if (($pay == "invoiceme" || eregi("avbetalning", $pay) || $this->mobilabonnemang || $pay == "faktura_kampanj" || $pay == "klarnakonto" || $this->aldersgrans) && isset($_SESSION['CKreditor']->cust_addresses) ) {
...
?>
			...
			<tr>
			  ...
			  <td align="center" bgcolor="#ECECE6"><font size="1" face="Verdana"><?php echo $count; ?></font></td>				  
...
function getMobilePhone($artnr) {
	...
	if (mysqli_num_rows($res) > 0) {
		...
		echo "<b>VÄLJ TELEFON I LISTAN</b>";
...
function getMobileOperator() {
	...
	$select = "SELECT DISTINCT mobile_plans.operator_id, operator.name FROM mobile_plans ";
	$select .= "LEFT JOIN operator ON mobile_plans.operator_id = operator.operator_id ";
	$select .= "WHERE validFrom < GETDATE() AND validTo > GETDATE() AND iswebstorefeatured = -1 AND isactive = -1 AND NOT (operator.name IS NULL) ";
	$res = mssql_query ($select, $this->conn_ms);
	...
	if (mssql_num_rows($res) > 0) {
		...
		echo "<td width=\"90\"><img border=\"0\" src=\"pic/choose_op.jpg\"></td>";
...
function getMobilePhonePicture($artnr) {
	...
	if (mysqli_num_rows($res) > 0) {
		...
		echo "<a href=\"info.php?article=$row->artnr\"><img border=\"0\" src=\"/thumbs/xlarge/bilder/".$row->bild ."\"</a>" ;
...
function getMobilePhoneList($artnr2) {
	...
	if (mysqli_num_rows($res) > 0) {
		...
		if (strlen($displayname) >= 40)
			$displayname = substr ($displayname, 0, 40) . "...";
...
function getOperatorAbbList($op) {
	global $abb;
	
	$select = "SELECT DISTINCT mobile_plans.name AS AbbNamn, operator.name FROM mobile_plans ";
	$select .= "LEFT JOIN operator ON mobile_plans.operator_id = operator.operator_id ";
	$select .= "WHERE validFrom < GETDATE() AND validTo > GETDATE() AND iswebstorefeatured = -1 AND isactive = -1 ";
	$select .= "AND mobile_plans.operator_id = '" . $op . "' ";
	...
	$res = mssql_query ($select, $this->conn_ms);
		...
		echo "<td width=\"98\"><img border=\"0\" src=\"pic/choose_abb.jpg\"></td>\n";
		echo "<td>";
...
function getAbbPlanLenght($abb) {
	...
	$select = "SELECT artnr, plan_length FROM mobile_plans ";
	$select .= "WHERE validFrom < GETDATE() AND validTo > GETDATE() AND iswebstorefeatured = -1 AND isactive = -1 AND name = '" . $abb . "' ";
	$res = mssql_query ($select, $this->conn_ms);
	...
	if (mssql_num_rows($res) > 0) {
		...
		echo "<td width=\"90\"><img border=\"0\" src=\"pic/choose_bind.jpg\"></td>\n";
...
function getPlanLength($artnr) {
	...
	$select = "SELECT plan_length FROM mobile_plans ";
	$select .= "WHERE artnr = '" . $artnr . "' ";
	$res = mssql_query ($select, $this->conn_ms);
	$row = mssql_fetch_object($res);
...
function getPlanDescription($artnr) {
	...
	$select = "SELECT description FROM mobile_plans ";
	$select .= "WHERE artnr = '" . $artnr . "' ";
	$res = mssql_query ($select, $this->conn_ms);
	$row = mssql_fetch_object($res);
...
function getAmtDiscount($bt) {
	...
	$select = "SELECT discountCustomerAmt FROM mobile_plans ";
