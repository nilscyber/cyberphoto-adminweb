...
function addNyhetsbrev($name, $email, $fi) {
	...
	if (@mssql_num_rows($res) == 0) {		
	    $s = "INSERT INTO Nyhetsbrev (email, name, dateAdded, confirmed, mailad) values (" .
	            "'" . $email . "', '" . $name . "', '" . date("Y-m-d H:i:s")  . "', 0, 0)";
	    //echo $s;	
	    @mssql_query($s);    
	}
}
...
function getArticleInfoBuy ($artnr) {
	...
	*/
	...
	if ($fi) 
		$select .= " && (utgangen_fi=0 OR lagersaldo_fi > 0 OR lagersaldo > 0 )  ";
	else
		$select .= " && (utgangen=0 OR lagersaldo > 0) ";
...
function viewItemsInBasket($firstbasket, $fi) {
	...
	if ($discountCode == "POSTPAKET" && $basketValue < 801) {
	...
	} elseif ($freight == "fraktpost" && $pay == "pf" && $discountCode == "POSTPAKET") {
		$discountCode = "POSTPAKETPF";
		if ($discountCode != "") {
			$select = "SELECT * FROM discountCodes WHERE discountCode = '" . $discountCode . "'";
			$res = mssql_query($select, $this->conn_ms);
			$rowDiscount = mssql_fetch_object($res);		
	...
	for ($i=0; ($i < $n);  $i+=2) {
		...
		//if ($fi) {
			...
			$select .= "frakt, lagersaldo_fi, bestallt_fi, Artiklar.release, Artiklar.kategori_id, Artiklar.tillverkar_id,  ";
			$select .= "lev_datum_fi, bestallningsgrans, lev_datum_norm, lev_datum_norm_fi, Artiklar.ej_brev as ej_brev, Kategori.ej_brev as kat_ej_brev, ";
				...
				if ($release != "") {
					...
					if (strtotime($release) >= mktime()) {					
						...
						if (!$sv) // TODO: finska på nästa rad
							print "<font size=\"1\" face=\"Verdana, Arial\" color=\"#85000D\"><a onMouseOver=\"return escape('Tuote on tilapäisesti lopussa. <br>Normaali toimitusaika on  <b>$lev_datum_norm_fi </b> päivää varastoomme<br>Tämä toimitusaika vaatii että toimittajalla on tuote varastossa')\" style=\"text-decoration: none\"> tillfälligt slut</font></a>" ;														
						else
...
function showDeliveryDate($date, $prec, $fi, $sv) {
	...
	if ($prec == 1 || $prec == "") { // exakt datum
		...
		if ($fi && !$sv)
			return "Oletettu saapumisaika viikko " .  strtolower (date("W", strtotime($date)));		
		else 	
		...
		if ($day > 0 && $day <= 10 ) {
			...
			if ($fi && !$sv)
				return "Oletettu saapumisaika " . $this->getMonthSv($month) . " puolivälissä";		
			else 
		...
		if ($fi && !$sv)
			return "Oletettu saapumisaika " . strtolower ( date("F", strtotime($date)));		
		else 
