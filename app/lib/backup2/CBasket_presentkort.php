...
function add_customer($uppdatera, $mailpassw) {
	...
	if (($pay == "invoiceme" || eregi("avbetalning", $pay) || $this->mobilabonnemang || $pay == "faktura_kampanj" || $pay == "klarnakonto" || $this->aldersgrans || $_SESSION['MOB_CONFIRMED']) && isset($_SESSION['CKreditor']->cust_addresses) ) {
...
function session_clear() {
	 ...
	 $_SESSION['old_nyhetsbrev'],$_SESSION['CcName'],$_SESSION['old_avtalskund'],$_SESSION['old_mobilnr'],$_SESSION['old_sms'],$_SESSION['old_nyhetsbrev'],
	 ...
	 $_SESSION['old_giftcardrebate'],$_SESSION['old_giftcardrebateown']);
...
function getArticleInfoBuy ($artnr) {
	...
	*/
	...
	$select .= "AND Artiklar.ej_med=0 && Artiklar.artnr='$artnr' ";
        ...
        $select .= "Artiklar.lagersaldo, Artiklar.bestallningsgrans, Artiklar.lev_datum_normal, Moms.momssats, Moms.momssats_fi, Artiklar.utgangen, Artiklar_fi.utgangen_fi, ";
        $select .= "Artiklar.artnr_lev, Artiklar.grossist_id, Leverantor.grossist, Leverantor.bestallare, Leverantor.telefon, Leverantor.ordertel ";
        $select .= "FROM Artiklar ";
        $select .= "INNER JOIN Tillverkare ON Artiklar.tillverkar_id = Tillverkare.tillverkar_id ";
        $select .= "INNER JOIN Leverantor ON Artiklar.grossist_id = Leverantor.grossist_id ";
...
function viewItemsInBasket($firstbasket, $fi) {
	...
	$PHP_SELF, $brev, $bestallningsgrans, $pack, $alltidBrev, $pallDelivery, $fi, $sv, $bestallt_fi, $lev_datum_norm_fi, $lev_datum_norm, $release, $discountCode, $discountCodeStatus,
	$freight, $pay, $articles, $discountAmount, $mobile, $old_presenkort, $old_giftcardrebate, $old_giftcardrebateown;	
	
	// echo $old_giftcardrebateown;
	...
	if ($_SESSION['MOB_CONFIRMED'] && $freight == "fraktpost") {
		$this->freeFreight = true;
	}
	...
	for ($i=0; ($i < $n);  $i+=2) {
		...
		*/
		...
		if ($firstbasket == 'nooutput') {
			...
			} elseif ($kategori_id == 560 || $kategori_id == 561 || $kategori_id == 567 || $kategori_id == 535 || $kategori_id == 549 || $kategori_id == 536 || $kategori_id == 547 || $kategori_id == 533 || $kategori_id == 534 || $kategori_id == 551 || $kategori_id == 537 || $kategori_id == 552 || $kategori_id == 532 || $kategori_id == 530 || $kategori_id == 540 || $kategori_id == 541) {
...
function check_lager_se($artnr) {
	...
	if ($_SERVER['REMOTE_ADDR'] == "192.168.1.89") { // endast test
