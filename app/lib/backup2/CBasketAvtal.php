...
function add_customer($uppdatera, $mailpassw) {
	...
	//return;
	if (($pay == "invoiceme" || eregi("avbetalning", $pay)) && isset($_SESSION['CKreditor']->cust_addresses) ) {
	...
	else
	{
	  if ($betalsatt == 'faktura') { $betalsatt_id = 4; }
	    else
	    {
	      ...
	      elseif ( eregi("avbetalning", $betalsatt)) $betalsatt_id = 7;
...
function getDiscountText($rowDiscount, $momssats, $utpris, $showDecoration) {
	...
	if ($fi) {
		...
		if ($rowDiscount->discountPercent != "") {
			...
			$txt = round($rowDiscount->discountAmount + $rowDiscount->discountAmount * $momssats, 0) .  " kr avdraget enligt rabattkod";
...
function getDiscountArticles2($articles, $fi) {
	 ...
	 * Obs! viewItemsInBasket must be run before this one! 
	 * 
...
function getDiscountArticles($articles, $fi) {
	...
	*/
	...
	foreach ($articles as $article)	{
		...
		$select = "SELECT discountCodes.*, discountCodeArticles.artnr as artnrMatch, discountCodeArticles.articleDiscounted FROM discountCodes, discountCodeArticles, Artiklar 
		WHERE Artiklar.artnr = discountCodeArticles.artnr AND discountCodeArticles.dscntId = discountCodes.cnt AND discountCodeArticles.artnr = '" . $article[0] . "' AND validDate > getdate() AND validFrom < getDate() " . $notSel . 
		" AND (Artiklar.utgangen = 0 OR Artiklar.lagersaldo > 0) ";
		//echo "<br>" . $select .  ": " . $article[1] . "<br>";
		$res = mssql_query($select, $this->conn_ms);
		...
		if (mssql_num_rows($res) > 0) {
			...
			while ($row = mssql_fetch_object($res)) {
				...
				$select = "SELECT * FROM discountCodeArticles WHERE dscntId = " . $row->cnt;
				...
				while ($rowDiscount = mssql_fetch_object($res2)) {		
					...
					$f = $this->search_array_n($rowDiscount->artnr, $articles2, 0); // varför matchar den inte sig själv här? (jo det gör den men det spelar ingen roll då det ändå testas nedanför om det är den artikel som skall rabatteras som kommit upp. Och det sparas också så inga dubletter skapas. 
					if ($f === false) {
						//echo "falskt!!";
						$testVar = false;
					} else { // 
						...
						//$this->obsafe_print_r($rowDiscount, false, true);
						...
						if ($rowDiscount->articleDiscounted == -1) {      // om det är en rabatterad vara, märk upp vilken spara											     
						...
						} elseif ($rowDiscount->includedArticle != "") {  // annars, om det är en vara som skall ingå gratis, 
							...
							array_push($arr, $rowDiscount->includedArticle);
				...
				if ($testVar) {
					...
					array_push($discountParts, $row->dscrptn); // beskrivningstext av rabatten (visas för kunden)
...
function getContractDiscountPart($contract_id, $artnr) {
	global $fi;
          
    $s = "SELECT AvtalDetalj.rabatt as rabatten FROM Artiklar, AvtalDetalj, Avtal" .
            " WHERE Avtal.avtals_id = " . $contract_id . " AND Artiklar.tillverkar_id = AvtalDetalj.tillverkar_id" .
            " AND Artiklar.kategori_id = AvtalDetalj.kategori_id" .
            " AND Avtal.avtals_id = AvtalDetalj.avtals_id AND Artiklar.artnr = '" . $artnr .  "'";

    
    $res = @mssql_query ($s, $this->conn_ms);
    $row = mssql_fetch_object($res);
    if (mssql_num_rows($res) == 0 || $row->rabatten == "") {
        $s = "SELECT AvtalDetalj.rabatt as rabatten FROM Artiklar, AvtalDetalj, Avtal" .
                " WHERE Avtal.avtals_id = " . $contract_id . " AND Artiklar.tillverkar_id = AvtalDetalj.tillverkar_id" .
                " AND Avtal.avtals_id = AvtalDetalj.avtals_id AND Artiklar.artnr = '" . $artnr . "'";
	    $res = @mssql_query ($s, $this->conn_ms);
        ...
        if (mssql_num_rows($res) == 0 || $row->rabatten == "") {
            $s = "SELECT AvtalDetalj.rabatt as rabatten FROM Artiklar, AvtalDetalj, Avtal" .
                    " WHERE Avtal.avtals_id = " . $contract_id .  " AND Artiklar.kategori_id = AvtalDetalj.kategori_id" .
                    " AND Avtal.avtals_id = AvtalDetalj.avtals_id AND Artiklar.artnr = '" . $artnr . "'";
		    $res = @mssql_query ($s, $this->conn_ms);
		    $row = mssql_fetch_object($res);
            if (mssql_num_rows($res) == 0 || $row->rabatten == "") 
...
function getContractDiscount($contract_id) {
	global $kundvagn, $fi, $avtalsRabatt, $old_avtalskund, $goodsvalue, $goodsvalueMoms;
	$avtalsRabatt = 0;
	$answers = $kundvagn;
	...
	for ($k=0; ($k < $n);  $k+=2) {
		...
		$arg = $argument[$k];        # Article id
		$count = $argument[$k+1];    # Keeps track of the number of the same article	
		//echo "<br>" . $arg;
		...
		$select .= "FROM Artiklar, Artiklar_fi, Moms WHERE Artiklar.artnr = Artiklar_fi.artnr_fi AND Artiklar.momskod = Moms.moms_id AND artnr='$arg'";
		//echo $select;
		...
		if (eregi("pac$", $row->artnr)) {
			...
			while ($row = mssql_fetch_object($res)):
			    ...
			    if ($i == $antalRader) {
					...
					// avtalsrabatten
					$rabattSats = 0;
					$rabattSats = $this->getContractDiscountPart($old_avtalskund, $row->artnr);
					//echo "<br>här: " . $rabattSats . " " . $row->artnr;
					//$avtalsRabatt += $rabattSats * $Totalt;
					$avtalsRabatt += $rabattSats * ($utpris - $Totalt);
					//echo "<br>rabattdel: " . $Totalt . ", " . $row->artnr;
					...
					$rabattSats = 0;
					$rabattSats = $this->getContractDiscountPart($old_avtalskund, $row->artnr);
					//echo "<br>här: " . $rabattSats . " " . $row->artnr . ", " . $andelKr;
					$avtalsRabatt += $rabattSats * ($utpris - $andelKr);										
					//echo "<br>här: " . $avtalsRabatt . " " . "";
					//$avtalsRabatt +=  * $andelKr;
					//echo "<br>rabattdel: " . $andelKr . ", " . $row->artnr;
			    }
			...
			//echo "här: " . $ny_rabatt;
		   		...
		   		//echo $avtalsRabatt . $arg . "<br>";
		   		$avtalsRabatt += $this->getContractDiscountPart($old_avtalskund, $arg) * $utpris;
		   		//echo $this->getContractDiscountPart($old_avtalskund, $arg) * $utpris . "<br>";
		   		//echo $avtalsRabatt . "<br>";
		   }
		   
	}
	
	$goodsvalue -= $avtalsRabatt;
	$goodsvalueMoms -= $avtalsRabatt*1.25;
...
function viewItemsInBasket($firstbasket, $fi) {
	...
	$PHP_SELF, $brev, $bestallningsgrans, $pack, $alltidBrev, $pallDelivery, $fi, $sv, $bestallt_fi, $lev_datum_norm_fi, $lev_datum_norm, $release, $discountCode, $discountCodeStatus, $freight, $pay, $basketValue, $articles, $discountAmount, $avtalsRabatt, $old_avtalskund;	
	...
	*/
	...
	if ($old_avtalskund != "" && ($firstbasket != 'nooutput') )
		$this->getContractDiscount($old_avtalskund);	
	if (ereg ("(grejor:)(.*)", $answers,$matches)) {
		# Split the number of items and article id s into a list
		$orderlista = $matches[2];
		$argument = split ("\|", $orderlista);
	...
	for ($i=0; ($i < $n);  $i+=2) {
		...
		if ($firstbasket == 'nooutput') {
			...
			if (eregi("pac$", $arg))  {
				...
				$select = "SELECT artnr_del as artnr, antal FROM Paketpriser WHERE artnr_paket = '" . $arg . "'";
				//echo "<br>" . $select .  ": " . "" . "<br>";
				$res = mysqli_query($this->conn_my, $select);
				if (mysqli_num_rows($res) > 0) {
					//echo "jo!";
...
?>
	...
	if ($extra_freight && $firstbasket == 'yes' && $firstbasket != 'nooutput' && $extra_freight != 999 && $extra_freight != 3 && $freight != Null) {
