...
"old_nyhetsbrev", "CcName", "avtalskund", "old_mobilnr", "old_sms", "old_nyhetsbrev", "visaMoms");
...
function login($kundnrtest, $passwd) {
	...
	*/
	...
	global $confirm, $salt, $kundnrsave, $avtalskund;
	$passwd = trim($passwd);
...
function customer_info() {
	...
	*/
	...
	$old_foretag, $old_userName, $avtalskund, $old_mobilnr, $old_sms, $old_nyhetsbrev, $försäkringsbolag;
	...
	if ($confirm == '1') {
		...
		$select .= "savelogin, land_fi, foretag, userName, avtalskund, mobilnr, sms, nyhetsbrev ";
		...
		if ($avtalskund > 0) {
			$select = "SELECT försäkringsbolag FROM Avtal WHERE avtals_id = " . $avtalskund;
			$res = mssql_query ($select);
			$row = mssql_fetch_object($res);
			$försäkringsbolag = $row->försäkringsbolag;
