...
function customer_info($lastInfo) {
	...
	*/
	...
	$freight, $pay, $fi, $sv, $old_villkor_id, $old_personnr, $old_firstName, $old_lastName, $old_firstNameDel, $old_lastNameDel, $old_foretag;
	...
	if ($confirm == 1) {
		...
		$select .= "firstName, lastName, firstNameDel, lastNameDel ";
...
function add_customer($uppdatera, $mailpassw) {
	...
	if (($pay == "invoiceme" || eregi("avbetalning", $pay)) && isset($_SESSION['CKreditor']->cust_addresses) ) {
		$new_lastName = $_SESSION['CKreditor']->cust_addresses[0][0];
		$new_co = $_SESSION['CKreditor']->cust_addresses[0][1];
		//$new_adres = $_SESSION['CKreditor']->cust_addresses[0][1];
		$new_postnr = $_SESSION['CKreditor']->cust_addresses[0][2];			
		$new_postadr = $_SESSION['CKreditor']->cust_addresses[0][3];		 		 		
		
		// TODO: lc/o ? 
		$new_lnamn = $_SESSION['CKreditor']->cust_addresses[0][0];
		$new_lco = $_SESSION['CKreditor']->cust_addresses[0][1];
		//$new_ladress = $_SESSION['CKreditor']->cust_addresses[0][1];
		$new_lpostnr = $_SESSION['CKreditor']->cust_addresses[0][2];			
		$new_lpostadr = $_SESSION['CKreditor']->cust_addresses[0][3];
		if ($fi) {
			$new_land = 'Finland';
			$new_lland = 'Finland';
		} else {
		...
		$new_lnamn = $new_firstName . " " . $new_lastName;
	...
	*/
	...
	if (!$uppdatera) {
		if ($this->testUsername ($userName)) {
			...
			$insert .= " skapad, nyhetsbrev, mobilnr, sms, lang, firstName, lastName, firstNameDel, lastNameDel, salary) values ";
			$insert .= "";
			...
			$insert .= ", '$new_firstName', '$new_lastName', '$new_firstNameDel', '$new_lastNameDel', $old_salary)";
			//echo $insert;
			if ($res2)
