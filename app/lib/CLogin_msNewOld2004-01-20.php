...
"paketref", "betalsatt", "spara_uppgifter", "old_faktlev", "intern", "old_foretag", "old_userName", "SesCc", "SesExpM", "SesExpY", "SesCcCode", "kortfax", "CcName");
...
function login($kundnrtest, $passwd) {
	...
	*/
	...
	if ($passwd == "") {
		...
		$select .= "WHERE kundnr like '$kundnrtest' ";
		...
		if (mssql_num_rows ($res) > 0) {
			...
			$select = "SELECT kundnr, kundid, userName, trials FROM Kund ";
			$select .= "WHERE userName = '$kundnrtest'";
...
function customer_info() {
	...
	*/
	...
	if ($confirm == '1') {
		...
		$select .= "savelogin, land_fi, foretag, userName ";
...
function add_customer($uppdatera, $mailpassw) {
	...
	$userName, $userTaken, $foretag, $new_userName;
	...
	*/
	...
	if ($uppdatera != 'yes') {
		...
		if (testUsername ($userName)) {
			...
			$insert .= "lnamn, lco, ladress, lpostadr, lpostnr, lland_id, kundid, lemail, ltelnr, faktlev, savelogin, userName, foretag) values ";
			...
			$insert .= "'$new_telnr', '$new_faktlev', '$spara', '$userName', '$foretaget')";
