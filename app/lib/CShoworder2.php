...
function show_old_order($kundnr) {
	...
	$select = "SELECT ordernr, inkommet, skickat, behandlat, leveranssatt_id, levklar, betalsatt_id FROM Ordertabell WHERE kundnr = '$kundnr' AND show_order = '0' order by ordernr DESC";
	...
	while ($row = mssql_fetch_array($res)):
		...
		// loopen före, därför: 
		$inkommet = $skickat = $behandlat = $levklar = NULL;
...
function levStatus ($skickat, $behandlat, $levklar, $betalsatt_id, $leveranssatt_id) {
	global $fi;
	setlocale (LC_ALL, 'en_US');
...
function getKollinrOnline ($ordernr, $land_id) {
	...
	if ($land_id == 46) {
		...
		$select  = "SELECT * FROM parcelNoCheck WHERE ordern = " . $ordernr;
		...
		if (mysqli_num_rows($res) > 0) {
			...
			if ($fi == 'yes') 
				$ret .= "<a href=\"kollinr2.php?ordernr_check=" . $ordernr . "\">" . $row->kollinr . "</a>" . " (Klikkaamalla tästä voit seurata pakettia)";
			else
				$ret .= "<a href=\"kollinr2.php?ordernr_check=" . $ordernr . "\">" . $row->kollinr . "</a>" . " (klicka för att följa paketet)";
				
			/*			
			if ($fi == 'yes') {
				$ret .= "<a href=\"kollinr2.php?ordernr_check=" . $ordernr . "\">Klikkaamalla tästä voit seurata pakettia</a>";
			}
			else {
				$ret .= "<a href=\"kollinr2.php?ordernr_check=" . $ordernr . "\">klicka för att följa paketet</a>";
			}
			*/
				$ret .= "<br>";
			endwhile;
