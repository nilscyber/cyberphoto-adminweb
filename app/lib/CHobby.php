...
?>
	<table border="0" cellpadding="0" cellspacing="0" width="160" height="215">
	  ...
	  <tr>
	    ...
	    <a onmouseover="return escape('<b><?php echo $produktbeskrivning; ?></b><?php if ($kortinfo != "") { ?><br><br><?php echo $kortinfo; ?><?php } ?><br><br>Pris: <b><?php echo $utprismoms; ?> kr</b> inkl. moms')" href="../info.php?article=<?php echo $artnr; ?>">
...
function mostSoldHobby($kategorier_id, $limit) {
	...
	*/
	...
	$output = "";
        ...
        $select  = "SELECT mostSoldArticles.artnr, mostSoldArticles.antalManad1, Artiklar.link, Artiklar.beskrivning, Artiklar.kortinfo, Artiklar.bild, Artiklar.lagersaldo, ";
        $select .= "Artiklar.bestallningsgrans, Artiklar.utpris, Artiklar.utpris_no, Tillverkare.tillverkare, Artiklar_fi.utpris_fi, Moms.momssats, Moms.momssats_fi, Moms.momssats_no, ";
	...
	if (mysqli_num_rows($res) > 0) {
		...
		if ($fi) {
			...
			$utprismoms = "Pris: " . number_format(($utpris_no + $utpris_no * $momssats_no), 0, ',', ' ') . "&nbsp;NOK";
		} else {
			$utprismoms = "Pris: " . number_format(($utpris + $utpris * $momssats), 0, ',', ' ') . "&nbsp;SEK";
		}
