...
function getList($extraSelect) {
	global $conn_my;
	$select  = "SELECT tillverkare, Artiklar.beskrivning, Artiklar_fi.beskrivning_fi, Artiklar.artnr, Artiklar.artnr_lev, utpris, Artiklar_fi.utpris_fi, kategori, kategori_fi, link, link2_fi as link_fi, lagersaldo, lagersaldo_fi, bild, Moms.momssats, Moms.momssats_fi, Artiklar.kategori_id FROM  ";
	...
	//$select .= " ej_med_fi=0 AND ej_med=0 AND (utgangen=0 OR lagersaldo > 0) AND demo=0 AND Artiklar.utpris > 0 "; //AND artnr not like '%pac' ";
	$select .= " (utgangen_fi=0 OR lagersaldo_fi > 0 OR lagersaldo > 0 ) AND demo=0 AND Artiklar_fi.utpris_fi > 0 AND ej_med_fi=0 AND ej_med=0 AND artnr not like '%pac' ";
	//$select .= "(  ";
...
function extr($row) {
	...
	$linken = "http://www.cyberphoto.fi/" . $linken;
