...
// Konfig (GET + förval föregående månad)
$dr_id      = 'purchase_kpi';
$dr_method  = 'get';
...
// (2) Hämta data
$rows = $statistics->getPurchaseStats($from, $to);
...
$total_orders = 0; $total_lines = 0; $total_totallines = 0.0;
...
<table style="width:100%;border-collapse:separate;border-spacing:0;font-size:14px">
  <thead>
    <tr>
      ...
      <th style="text-align:right;padding:8px;border-bottom:2px solid #eee">Antal inköpsordrar</th>
      <th style="text-align:right;padding:8px;border-bottom:2px solid #eee">Antal orderrader totalt</th>
      <th style="text-align:right;padding:8px;border-bottom:2px solid #eee">Total Lines (summa)</th>
    </tr>
  </thead>
  <tbody>
    <?php if ($rows): foreach ($rows as $r): ?>
      <tr>
        ...
        <td style="padding:8px;border-bottom:1px solid #f1f1f1;text-align:right"><?php echo $fmtInt($r['antal_inkopsordrar']); ?></td>
    ...
    <?php endforeach; else: ?>
      <tr><td colspan="4" style="padding:12px;text-align:center;color:#666">Inga träffar för valt intervall.</td></tr>
    <?php endif; ?>
    <?php if ($rows): ?>
      <tr>
