<?php
// ============================================================
// standard_locator_v1.php
// Sida: Begagnade produkter med lagerplats Standard
// Visar begagnade produkter (istradein='Y') som:
//   - Har lagerplats Standard (m_locator_id = 1000000)
//   - Har mer an 0 i lager
//   - Vars salestart har passerats
// Sorteras pa salestart ASC (aldst overst)
// ============================================================

include_once("top.php");
require_once("CUsedProducts.php");
...
$rows = $tool->getStandardLocatorProducts();
...
.age-chip.age-warn  { background: #fef3c7; color: #92400e; border: 1px solid #fcd34d; }
.age-chip.age-alert { background: #ffe4e6; color: #9f1239; border: 1px solid #fca5a5; }
.age-chip.age-old   { background: #fecaca; color: #7f1d1d; border: 1px solid #ef4444; font-weight: 900; }
.age-chip.age-new   { background: #f0fdf4; color: #166534; border: 1px solid #86efac; }
.copy-art {
    cursor: pointer;
    user-select: none;
    padding: 1px 4px;
...
.table-list td a { color: #1d4ed8; text-decoration: none; }
...
<div style="padding: 16px;">
    ...
    <h1>Begagnade produkter med lagerplats Standard</h1>

    <div class="result-bar">
        <div class="result-info">
            <strong><?php echo $total; ?> st</strong> produkter med lagerplats Standard
        </div>
    </div>
    ...
    <?php if ($total === 0): ?>
        ...
        <div class="no-results">
            Inga begagnade produkter med lagerplats Standard hittades.
        </div>
        ...
        <table class="table-list">
            ...
            <thead>
                <tr>
                    <th>Artnr</th>
                    <th>Produkt</th>
                    <th class="text-right">&Aring;lder (dagar)</th>
            ...
            <?php foreach ($rows as $r):
                ...
                if ($ageDays > 365)      { $ageClass = 'age-old'; }
                elseif ($ageDays > 180)  { $ageClass = 'age-alert'; }
                ...
                <tr>
                    ...
                    <td>
                        <a href="<?php echo $h($drawerUrl); ?>"
                           target="_blank"
                           rel="noopener"
                           title="S&auml;ljstart: <?php echo $salestart; ?>">
...
(function() {
    function copyText(txt, cb) {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(String(txt || '')).then(
                function() { cb && cb(true); }, function() { cb && cb(false); }
            );
        } else {
            try {
                ...
                document.body.removeChild(ta); cb && cb(ok);
            } catch (e) { cb && cb(false); }
        }
    }
    document.addEventListener('click', function(e) {
        ...
        copyText(art, function(ok) {
            ...
            setTimeout(function() { el.classList.remove('copied'); el.setAttribute('title', oldTitle); }, 1000);
        });
    }, false);
})();
</script>
