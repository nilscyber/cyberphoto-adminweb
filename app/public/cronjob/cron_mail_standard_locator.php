<?php
// /cronjob/cron_mail_standard_locator.php
// Skickar sammanfattning av begagnade produkter med lagerplats Standard.
...
$rcpt = CCronMailRecipients::get('standard_locator');
$to   = $rcpt['to'];
$bcc  = $rcpt['bcc'];
...
$dtStr       = date('Y-m-d');
$subjectText = "Begagnade produkter - lagerplats Standard - {$total} st ({$dtStr})";
$subject     = '=?UTF-8?B?' . base64_encode($subjectText) . '?=';

$html  = '<!doctype html><html><head><meta charset="UTF-8"></head>';
$html .= '<body style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#111;">';
$html .= '<p>Begagnade produkter med lagerplats <strong>Standard</strong> (m_locator_id = 1000000) med lager &gt; 0 vars s&auml;ljstart har passerats.</p>';
$html .= '<p><strong>' . $total . ' st</strong> produkter.</p>';
...
foreach ($rows as $r) {
    ...
    elseif ($ageDays > 90)  { $ageBg = '#fef3c7'; }
    else                    { $ageBg = '#f0fdf4'; }
...
$html .= '</tr>';
$html .= '</table>';
$html .= '<p style="margin-top:12px;"><a href="https://admin.cyberphoto.se/standard_locator.php">Visa i admin</a></p>';
$html .= '</body></html>';

$boundary = '=_CP_STD_' . md5(uniqid((string)mt_rand(), true));
$headers   = array();
